...
func (storm *StormChain) startBloomHandlers(sectionSize uint64) {
	for i := 0; i < bloomServiceThreads; i++ {
		go func() {
			for {
				select {
				case <-storm.shutdownChan:
					return
				...
				case request := <-storm.bloomRequests:
					...
					for i, section := range task.Sections {
						head := rawdb.ReadCanonicalHash(storm.chainDb, (section+1)*sectionSize-1)
						if compVector, err := rawdb.ReadBloomBits(storm.chainDb, task.Bit, section, head); err == nil {
...
// for the StormChain header bloom filters, permitting blazing fast filtering.
