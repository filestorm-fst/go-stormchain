...
func (p *peer) Info() *storm.PeerInfo {
	return &storm.PeerInfo{
		Version:    p.version,
		Difficulty: p.Td(),
...
func (p *peer) Handshake(td *big.Int, head common.Hash, headNum uint64, genesis common.Hash, server *LesServer) error {
	...
	if server != nil {
		...
		if !server.config.UltraLightOnlyAnnounce {
			...
			// If local stormchain node is running in archive mode, advertise ourselves we have
...
// the Light StormChain sub-protocol.
type peerSet struct {
