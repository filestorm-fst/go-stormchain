...
import (
	...
	"github.com/filestorm-fst/go-stormchain/p2p/simulations"
	"github.com/filestorm-fst/go-stormchain/p2p/simulations/adapters"
...
func newLesServerService(ctx *adapters.ServiceContext) (node.Service, error) {
	...
	stormchain, err := storm.New(ctx.NodeContext, &config)
	if err != nil {
		return nil, err
	}
	server, err := NewLesServer(stormchain, &config)
	if err != nil {
		return nil, err
	}
	stormchain.AddLesServer(server)
	return stormchain, nil
}
