...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.stormchain", new NodeConfig());
			node.start();
			...
			StormChainClient ec = node.getStormChainClient();
	...
	// signatures too. Regression test for go-stormchain issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altostormer
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/stormchain/stormtest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.stormchain", "github.com/filestorm-fst/go-stormchain/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "stormchain", "stormtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.stormchain.stormtest"
