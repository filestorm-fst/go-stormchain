...
import (
	...
	"github.com/filestorm-fst/go-stormchain/accounts/usbwallet/trezor"
...
// or whether the StormChain app was not started on it.
...
// StormChain address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.StormChainAddress)
	if _, err := w.trezorExchange(&trezor.StormChainGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.StormChainSignTx{
	...
	response := new(trezor.StormChainTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.StormChainTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
