...
// Package filters implements an filestorm filtering system for block,
...
type subscription struct {
	...
	typ       Type
	created   time.Time
	logsCrit  filestorm.FilterQuery
	logs      chan []*types.Log
...
func (es *EventSystem) SubscribeLogs(crit filestorm.FilterQuery, logs chan []*types.Log) (*Subscription, error) {
...
func (es *EventSystem) subscribeMinedPendingLogs(crit filestorm.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribeLogs(crit filestorm.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribePendingLogs(crit filestorm.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
