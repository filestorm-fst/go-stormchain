...
import (
	...
	"github.com/filestorm-fst/go-stormchain/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute StormChain requests through
...
func (c *Console) init(preload []string) error {
	...
	flatten := "var storm = web3.storm; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = storm.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines storm.getBalance(storm.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
func (c *Console) Welcome() {
	...
	if res, err := c.jsre.Run(`
		...
		try {
			message += "coinbase: " + storm.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + storm.blockNumber + " (" + new Date(1000 * storm.getBlock(storm.blockNumber).timestamp) + ")\n";
		try {
