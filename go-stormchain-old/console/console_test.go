...
import (
	...
	"github.com/filestorm-fst/go-stormchain/internal/jsre"
...
func newTester(t *testing.T, confOverride func(*storm.Config)) *tester {
	...
	// Create a networkless protocol stack and start an StormChain service within
	stack, err := node.New(&node.Config{DataDir: workspace, UseLightweightKDF: true, Name: testInstance})
	if err != nil {
		t.Fatalf("failed to create node: %v", err)
	}
	ethConf := &storm.Config{
	...
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return storm.New(ctx, ethConf) }); err != nil {
		t.Fatalf("failed to register StormChain protocol: %v", err)
	}
	...
	stack.Service(&stormchain)

	return &tester{
		workspace: workspace,
		stack:     stack,
		stormchain:  stormchain,
		console:   console,
		input:     prompter,
		output:    printer,
	}
