...
var (
	...
	maxQueuedHeaders         = 32 * 1024                    // [storm/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	// Channels
	headerCh      chan dataPack        // [storm/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [storm/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [storm/63] Channel receiving inbound receipts
	bodyWakeCh    chan bool            // [storm/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [storm/63] Channel to signal the receipt fetcher of new tasks
	headerProcCh  chan []*types.Header // [storm/62] Channel to feed the header processor new tasks

	// for stateFetcher
	stateSyncStart chan *stateSync
	trackStateReq  chan *stateReq
	stateCh        chan dataPack // [storm/63] Channel receiving inbound node state data
...
// these are zero.
func (d *Downloader) Progress() stormchain.SyncProgress {
	...
	return stormchain.SyncProgress{
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "storm", p.version, "head", hash, "td", td, "mode", d.mode)
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	if index, err := d.blockchain.InsertChain(blocks); err != nil {
		if index < len(results) {
			...
			// The importer will put tostormer a new list of blocks to import, which is a superset
