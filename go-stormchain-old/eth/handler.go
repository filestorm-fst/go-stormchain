...
import (
	...
	"github.com/filestorm-fst/go-stormchain/storm/fetcher"
...
// NewProtocolManager returns a new StormChain sub protocol manager. The StormChain sub protocol manages peers capable
// with the StormChain network.
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing StormChain peer", "peer", id)

	// Unregister the peer from the downloader and StormChain peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping StormChain protocol")
	...
	log.Info("StormChain protocol stopped")
}
...
// handle is the callback invoked to manage the life cycle of an storm peer. When
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("StormChain peer connected", "name", p.Name())

	// Execute the StormChain handshake
	var (
	...
	if err := p.Handshake(pm.networkID, td, hash, genesis.Hash(), forkid.NewID(pm.blockchain), pm.forkFilter); err != nil {
		p.Log().Debug("StormChain handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("StormChain peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("StormChain message handling failed", "err", err)
			return err
		}
	}
}
...
// NodeInfo represents a short summary of the StormChain sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // StormChain network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
