...
// nodeDockerfile is the Dockerfile required to run an StormChain node.
var nodeDockerfile = `
FROM stormchain/client-go:latest
...
RUN \
  echo 'storm --cache 512 init /genesis.json' > storm.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.stormchain/keystore/ && cp /signer.json /root/.stormchain/keystore/' >> storm.sh && \{{end}}
	echo $'exec storm --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.etherbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> storm.sh

ENTRYPOINT ["/bin/sh", "storm.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an StormChain node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.stormchain{{if .Ethashdir}}
...
// deployNode deploys a new StormChain node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.stormchain"],
