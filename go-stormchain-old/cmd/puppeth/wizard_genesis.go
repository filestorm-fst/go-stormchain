...
func (w *wizard) manageGenesis() {
	...
	case "2":
		...
		stormJson := filepath.Join(folder, fmt.Sprintf("%s.json", w.network))
		if err := ioutil.WriteFile((stormJson), out, 0644); err != nil {
			log.Error("Failed to save genesis file", "err", err)
			return
		}
		log.Info("Saved native genesis chain spec", "path", stormJson)

		// Export the genesis spec used by Aleth (formerly C++ StormChain)
		...
		// Export the genesis spec used by Harmony (formerly StormChainJ)
