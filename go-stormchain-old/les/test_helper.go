...
import (
	...
	"github.com/filestorm-fst/go-stormchain/accounts/abi/bind/backends"
	...
	"github.com/filestorm-fst/go-stormchain/contracts/checkpointoracle/contract"
	...
	"github.com/filestorm-fst/go-stormchain/les/flowcontrol"
...
func testIndexers(db ethdb.Database, odr light.OdrBackend, config *light.IndexerConfig) []*core.ChainIndexer {
	...
	indexers[1] = storm.NewBloomIndexer(db, config.BloomSize, config.BloomConfirms)
...
func newTestClientHandler(backend *backends.SimulatedBackend, odr *LesOdr, indexers []*core.ChainIndexer, db ethdb.Database, peers *peerSet, ulcServers []string, ulcFraction int) *clientHandler {
	...
	client := &LightStormChain{
		lesCommons: lesCommons{
			genesis:     genesis.Hash(),
			config:      &storm.Config{LightPeers: 100, NetworkId: NetworkId},
