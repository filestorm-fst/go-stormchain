...
import (
	...
	"github.com/filestorm-fst/go-stormchain/accounts/external"
	"github.com/filestorm-fst/go-stormchain/accounts/keystore"
	"github.com/filestorm-fst/go-stormchain/accounts/scwallet"
	"github.com/filestorm-fst/go-stormchain/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of storm is "storm". If no
...
func (c *Config) NodeName() string {
	...
	if name == "storm" || name == "storm-testnet" {
		name = "Geth"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "storm" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	// by storm 1.4 are used if they exist.
	if warn, isOld := isOldGethResource[path]; isOld {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGethResourceWarning, "Using deprecated resource file %s, please move this file to the 'storm' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-stormchain-keystore")
		ephemeral = keydir
	}
