...
import (
	...
	"github.com/filestorm/go-filestorm/internal/testlog"
...
func TestRemoteNotify(t *testing.T) {
	...
	// Create the custom fstash engine.
	fstash := NewTester([]string{server.URL}, false)
	defer fstash.Close()
	...
	fstash.Seal(nil, block, nil, nil)
	select {
	case work := <-sink:
		if want := fstash.SealHash(header).Hex(); work[0] != want {
...
func TestRemoteMultiNotify(t *testing.T) {
	...
	// Create the custom fstash engine.
	fstash := NewTester([]string{server.URL}, false)
	fstash.config.Log = testlog.Logger(t, log.LvlWarn)
	defer fstash.Close()
...
func TestStaleSubmission(t *testing.T) {
	...
	for id, c := range testcases {
		for _, h := range c.headers {
			fstash.Seal(nil, types.NewBlockWithHeader(h), results, nil)
		}
		if res := api.SubmitWork(fakeNonce, fstash.SealHash(c.headers[c.submitIndex]), fakeDigest); res != c.submitRes {
		...
		case <-time.NewTimer(time.Second).C:
			t.Errorf("case %d fetch fstash result timeout", id+1)
		}
	}
}
