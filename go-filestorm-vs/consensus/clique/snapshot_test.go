...
// mapped from textual names used in the tests below to actual Filestorm private
...
// address retrieves the Filestorm address of a tester account by label, creating
...
func (ap *testerAccountPool) address(account string) common.Address {
	...
	// Resolve and return the Filestorm address
...
func TestClique(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			// batch (https://github.com/filestorm/go-filestorm/issues/17593). Whilst this
	...
	for i, tt := range tests {
		...
		for j, block := range blocks {
			// Storm the header and prepare it for signing
			header := block.Header()
			if j > 0 {
