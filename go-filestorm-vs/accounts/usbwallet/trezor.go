...
import (
	...
	"github.com/filestorm/go-filestorm/accounts/usbwallet/trezor"
...
// or whether the Filestorm app was not started on it.
...
// Filestorm address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.FilestormAddress)
	if _, err := w.trezorExchange(&trezor.FilestormGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.FilestormSignTx{
	...
	response := new(trezor.FilestormTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.FilestormTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
