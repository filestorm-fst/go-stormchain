...
func (set *unconfirmedBlocks) Insert(index uint64, hash common.Hash) {
	...
	log.Info("Mined a potential block", "block", index, "hash", hash)
}
...
func (set *unconfirmedBlocks) Shift(height uint64) {
	...
	for set.blocks != nil {
		...
		case header.Hash() == next.hash:
			log.Info("Reached canonical chain", "block", next.index, "hash", next.hash)
		default:
			...
			if included {
				log.Info("Confirmed other block", "block", next.index, "hash", next.hash)
			} else {
				log.Info("Throw away a block", "block", next.index, "hash", next.hash)
			}
		}
