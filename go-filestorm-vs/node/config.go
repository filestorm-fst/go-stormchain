...
import (
	...
	"github.com/filestorm/go-filestorm/accounts/external"
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Storm", keep that.
...
var isOldStormResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	if warn, isOld := isOldStormResource[path]; isOld {
		oldpath := ""
		if c.name() == "storm" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldStormResourceWarning, "Using deprecated resource file %s, please move this file to the 'storm' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-filestorm-keystore")
		ephemeral = keydir
	}
