...
// Package les implements the Light MoacService Subprotocol.
package les
...
func NewLesServer(mc *mc.MoacService, config *mc.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(mc.BlockChain().Config(), false, config.NetworkId, mc.EventMux(), mc.Engine(), newPeerSet(), mc.BlockChain(), mc.TxPool(), mc.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	pm.blockLoop()
	...
	srv := &LesServer{
		protocolManager: pm,
		quitSync:        quitSync,
		lesTopic:        lesTopic(mc.BlockChain().Genesis().Hash()),
	}
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(mc.ChainDb())
	return srv, nil
}
...
func newCostStats(db mcdb.Database) *requestCostStats {
...
func getChtRoot(db mcdb.Database, num uint64) common.Hash {
...
func storeChtRoot(db mcdb.Database, num uint64, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], num)
	db.Put(append(chtPrefix, encNumber[:]...), root[:])
}

func makeCht(db mcdb.Database) bool {
