...
func newServerPool(db mcdb.Database, quit chan struct{}, wg *sync.WaitGroup) *serverPool {
	pool := &serverPool{
		db:           db,
		quit:         quit,
		wg:           wg,
...
func (pool *serverPool) dial(entry *poolEntry, knownSelected bool) {
	...
	entry.dialed = addr
	go func() {
		pool.server.AddPeer(discover.NewNode(entry.id, addr.ip, addr.port, addr.port, &addr.benefialAddress, &addr.serviceCfg, addr.showToPublic, &addr.ipString))
		select {
		case <-pool.quit:
...
func (e *poolEntry) DecodeRLP(s *rlp.Stream) error {
	...
	addr := &poolEntryAddress{ip: entry.IP, port: entry.Port, fails: entry.Fails, lastSeen: mclock.Now(), benefialAddress: entry.BeneficialAddress, serviceCfg: entry.ServiceCfg}
	e.id = entry.ID
...
type poolEntryAddress struct {
	...
	benefialAddress common.Address
	serviceCfg string
	showToPublic bool
	ipString string
}
