...
const (
	...
	ethVersion = 63 // equivalent MOAC version for the downloader
...
// NewProtocolManager returns a new ethereum sub protocol manager. The MoacNode sub protocol manages peers capable
// with the ethereum network.
func NewProtocolManager(chainConfig *params.ChainConfig, lightSync bool, networkId uint64, mux *event.TypeMux, engine consensus.Engine, peers *peerSet, blockchain BlockChain, txpool txPool, chainDb mcdb.Database, odr *LesOdr, txrelay *LesTxRelay, quitSync chan struct{}, wg *sync.WaitGroup) (*ProtocolManager, error) {
	...
	if disableClientRemovePeer {
		removePeer = func(id string, subnet string) {}
	}
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light MoacNode protocol")
	...
	log.Info("Light MoacNode protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light MoacNode peer registration failed", "err", err)
		return err
	}
	defer func() {
		...
		pm.removePeer(p.id, string(p.Peer.Subnet()))
	}()
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light MoacNode message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light MoacNode message arrived", "code", msg.Code, "bytes", msg.Size)
...
func (self *ProtocolManager) NodeInfo() *mc.MoacNodeInfo {
	return &mc.MoacNodeInfo{
