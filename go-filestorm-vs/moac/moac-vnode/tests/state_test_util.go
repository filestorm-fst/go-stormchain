...
func (t *StateTest) Run(subtest StateSubtest, vmconfig vm.Config) (*state.StateDB, error) {
	// log.Info("[tests/state_test_util.go->Run in]")
	...
	context := core.NewEVMContext(msg, block.Header(), nil, &t.json.Env.Coinbase, nil)
	...
	if logs := common.RlpHash(statedb.Logs()); logs != common.Hash(post.Logs) {
...
func makePreState(db mcdb.Database, accounts core.GenesisAlloc) *state.StateDB {
...
func (tx *stTransaction) toMessage(ps stPostState) (core.Message, error) {
	...
	msg := types.NewMessage(from, to, tx.Nonce, value, new(big.Int).SetUint64(gasLimit), tx.GasPrice, data, false, false, true, nil, nil, 0)
	return msg, nil
}
