...
const (
	...
	testAddressMoac = "mQ6ArdtrA2dPSLMNo2DX6jkQxUHqoeL"
)
...
func newTester(t *testing.T, confOverride func(*mc.Config)) *tester {
	...
	// Create a networkless protocol stack and start an MoacNode service within
	...
	mcConf := &mc.Config{
		...
		Moacbase: common.HexToAddress(testAddress),
		PowTest:  true,
	}
	fmt.Println("mcbase", mcConf.Moacbase.GetBase58Str())

	if confOverride != nil {
		confOverride(mcConf)
		//fmt.Println("Override with mcConf", mcConf.PowTest)
	}
	fmt.Println("Register the node with current settings!")

	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return mc.New(ctx, mcConf) }); err != nil {
		t.Fatalf("failed to register MoacNode protocol: %v", err)
	}
	// fmt.Println("After register, start the node......", mc.MoacNode.) //Cannot identify
	...
	fmt.Println("Attaching the node......")
	...
	fmt.Println("Create the console after node......")
	...
	var moacnode *mc.MoacNode
	stack.Service(&moacnode)
...
func TestWelcome(t *testing.T) {
	...
	fmt.Println("After init new tester......")
