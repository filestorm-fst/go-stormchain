...
import (
	...
	"github.com/filestorm/go-filestorm/moac/moac-vnode/accounts/usbwallet"
...
func newBridge(client *rpc.Client, prompter UserPrompter, printer io.Writer) *bridge {
	log.Debug("[console/bridge.go->newBridge]")
	return &bridge{
		client:   client,
		prompter: prompter,
		printer:  printer,
...
// RPC method (saved in jmc.newAccount) with it to actually execute the RPC call.
func (b *bridge) NewAccount(call otto.FunctionCall) (response otto.Value) {
	log.Debug("[console/bridge.go->NewAccount]")
	var (
		password string
		confirm  string
		err      error
	...
	ret, err := call.Otto.Call("jmc.newAccount", nil, password)
	if err != nil {
		throwJSException(err.Error())
	}
	return ret
...
func (b *bridge) OpenWallet(call otto.FunctionCall) (response otto.Value) {
	log.Debug("[console/bridge.go->OpenWallet]")
	...
	val, err := call.Otto.Call("jmc.openWallet", nil, wallet, passwd)
	if err == nil {
		return val
	}
	...
	if val, err = call.Otto.Call("jmc.openWallet", nil, wallet, passwd); err != nil {
		throwJSException(err.Error())
	}
	return val
}
...
// original RPC method (saved in jmc.unlockAccount) with it to actually execute
// the RPC call.
func (b *bridge) UnlockAccount(call otto.FunctionCall) (response otto.Value) {
	log.Debug("[console/bridge.go->UnlockAccount]")
	...
	//inVal := fmt.Sprintf("%s", call.Argument(0))
	//var account string
	//
	////Add check of base58 encoding
	//if common.IsMoacAddress(inVal) {
	//	account = common.MoacToAddress(inVal).Hex()
	//} else if common.IsHexAddress(inVal) {
	//	account = common.HexToAddress(inVal).Hex()
	//} else {
	//	errors.New("Unknown argument")
	//}
	...
	val, err := call.Otto.Call("jmc.unlockAccount", nil, account, passwd, duration)
	if err != nil {
		throwJSException(err.Error())
	}
	return val
...
// jmc.sign) with it to actually execute the RPC call.
func (b *bridge) Sign(call otto.FunctionCall) (response otto.Value) {
	// log.Debug("[console/bridge.go->Sign]")
	var (
	...
	// Send the request to the backend and return
	val, err := call.Otto.Call("jmc.sign", nil, message, account, passwd)
	if err != nil {
		throwJSException(err.Error())
	}
	return val
...
func (b *bridge) Sleep(call otto.FunctionCall) (response otto.Value) {
	log.Debug("[console/bridge.go->Sleep]")
...
func (b *bridge) SleepBlocks(call otto.FunctionCall) (response otto.Value) {
	log.Debug("[console/bridge.go->SleepBlocks]")
	var (
		blocks = int64(0)
	...
	// go through the console, this will allow chain3 to call the appropriate
	// callbacks if a delayed response or notification is received.
	blockNumber := func() int64 {
		result, err := call.Otto.Run("mc.blockNumber")
		if err != nil {
			throwJSException(err.Error())
		}
...
// Send implements the chain3 provider "send" method.
func (b *bridge) Send(call otto.FunctionCall) (response otto.Value) {
	log.Debug("[console/bridge.go->Send]")
	...
	log.Debug("[console/bridge.go->Send] rawReq=" + rawReq)
	...
	log.Debug("[console/bridge.go->Send] location 1")
	...
	for _, req := range reqs {
		...
		for param := range req.Params {
			log.Debug("[console/bridge.go->Send] param=" + string(param))
		}
	...
	log.Debug("[console/bridge.go->Send] location 2")
	...
	log.Debug("[console/bridge.go->Send] location 3")
	return response
}
