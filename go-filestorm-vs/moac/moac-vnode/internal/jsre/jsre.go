...
import (
	...
	"github.com/filestorm/go-filestorm/moac/moac-vnode/internal/jsre/deps"
	"github.com/robertkrimen/otto"
)

var (
	BigNumber_JS = deps.MustAsset("bignumber.js")
	Chain3_JS      = deps.MustAsset("chain3.js")
)
...
func (self *JSRE) Run(in_code string) (v otto.Value, err error) {
	
	self.Do(func(vm *otto.Otto) { v, err = vm.Run(in_code) })
	return v, err
}

// Get returns the value of a variable in the JS environment.
func (self *JSRE) Get(out_ns string) (v otto.Value, err error) {
	self.Do(func(vm *otto.Otto) { v, err = vm.Get(out_ns) })
	return v, err
}

// Set assigns value v to a variable in the JS environment.
func (self *JSRE) Set(in_ns string, v interface{}) (err error) {
	self.Do(func(vm *otto.Otto) { err = vm.Set(in_ns, v) })
	return err
}
