...
contract SubChainBase {
    ...
    uint public rewardPerBlk;
    uint public availableCoin;
    ...
    function updateRewardPerBlk(uint num) public {
        require(owner == msg.sender);
    ...
    function rewardSCS(bytes32 hash ) private {
        Proposal storage prop = proposals[hash];
        
        uint reward = rewardPerBlk * (prop.end - prop.start + 1);
        if (availableCoin > reward) {
            availableCoin -= reward;
            TestCoin erc20 = TestCoin(ERCAddr);
            erc20.transfer(prop.proposedBy, reward);
        }
    }
    ...
    function addRewardErc20(uint256 _value) public returns (bool) {
        if (ERCAddr != address(0)) {
            TestCoin erc20 = TestCoin(ERCAddr);
            uint256 allowedamt = erc20.allowance(msg.sender, address(this));
            if (_value <= allowedamt) {
                availableCoin += _value;
                return erc20.transferFrom(msg.sender, address(this), _value);
            }
        }
        return false;
    }
