...
contract SubChainBase {
    ...
    //constructor
    function SubChainBase(address proto, address vnodeProtocolBaseAddr, uint256 min, uint256 max, uint256 thousandth, uint256 flushRound, uint256 tokensupply, uint256 exchangerate) public {
    ...
    function requestProposalAction(uint256 indexInlist, bytes32 hash) public returns (bool) {
        ...
        totalOperation = this.balance - totalExchange/priceOneGInMOAC - totalBond;
    ...
    function settlement() public {
        ...
        if (block.number > blk) {
            foundation.transfer(totalExchange/priceOneGInMOAC);
        }
    }
