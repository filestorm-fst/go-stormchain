...
contract SubChainBase {
    ...
    uint256 public per_redeemdata_num = 130;//140
    ...
    uint256 public moacReserve;
    uint256 public Lockmoac;
    uint32 public weight;
    uint256 public totalOperation;
    uint256 public totalBond;
    
    address public FormulaAddr;
    ...
    //constructor
    function SubChainBase(address proto, address vnodeProtocolBaseAddr, address formulaAddr, uint32 _weight, uint256 initialTokenNum, uint256 min, uint256 max, uint256 thousandth, uint256 flushRound) public payable {
        ...
        weight = _weight;
        moacReserve = msg.value;
        Lockmoac = msg.value;
        totalToken = initialTokenNum * 10 ** 18;
        FormulaAddr = formulaAddr;
        // holdingPool.userAddr.push(msg.sender);
        // holdingPool.amount.push(totalToken);
        ...
        Formula bnt  = Formula(FormulaAddr);
        BALANCE = bnt.calculatePurchaseReturn(totalToken, moacReserve, weight, 1.5 * 10 ** 26 - moacReserve) + totalToken;
    ...
    function getEstFlushBlock(uint256 index) public view returns (uint256) {
        ...
        //     uint256 jump = (curblk-blk)/(2 * proposalExpiration * nodeCount);
    ...
    function requestProposalAction(uint256 indexInlist, bytes32 hash) public returns (bool) {
        ...
        totalOperation = this.balance - totalBond - moacReserve;
        
        //refund all to owner
        if (subchainstatus == uint256(SubChainStatus.close)) {
            owner.transfer(this.balance - totalBond - moacReserve);
            totalOperation = 0;
        }
        return true;
    }
    ...
    function requestEnterAndRedeemAction(bytes32 hash) public returns (bool) {
        ...
        if (prop.redeemAgreeList.length > nodeCount/2 && prop.preRedeemNum != 0) {
            ...
            for (i = pos; i < pos + len; i++) {
                ...
                uint256 moac = bnt.calculateSaleReturn(totalToken, moacReserve, weight, amt);
                addr.transfer(moac);
                moacReserve -= moac;
                totalToken -= amt;
    ...
    function buyMintToken() public payable returns (bool){
        Formula bnt  = Formula(FormulaAddr);
        uint256 token = bnt.calculatePurchaseReturn(totalToken, moacReserve, weight, msg.value);
        moacReserve += msg.value;
        totalToken += token;
    ...
    function buyMintTokenTransfer(address _to) public payable returns (bool){
        Formula bnt  = Formula(FormulaAddr);
        uint256 token = bnt.calculatePurchaseReturn(totalToken, moacReserve, weight, msg.value);
        moacReserve += msg.value;
        totalToken += token;
    ...
    function settlement() public {
        ...
        if (block.number > blk) {
            foundation.transfer(moacReserve);
        }
    }
