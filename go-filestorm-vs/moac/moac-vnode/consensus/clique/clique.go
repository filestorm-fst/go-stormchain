...
// ecrecover extracts the MoacNode account address from a signed header.
func ecrecover(header *types.Header, sigcache *lru.ARCCache) (common.Address, error) {
	...
	// Recover the public key and the MoacNode address
...
// MoacNode testnet following the Ropsten attacks.
type Clique struct {
	config *params.CliqueConfig // Consensus engine configuration parameters
	db     mcdb.Database        // Database to store and retrieve snapshot checkpoints
	...
	signer common.Address // MoacNode address of the signing key
...
func New(config *params.CliqueConfig, db mcdb.Database) *Clique {
...
// Author implements consensus.Engine, returning the MoacNode address recovered
...
func (c *Clique) Finalize(chain consensus.ChainReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header, receipts []*types.Receipt, liveFlag bool) (*types.Block, error) {
	log.Debugf("[consensus/clique/clique.go->Finalize]")
	// No block rewards in PoA, so the state remains as is and uncles are dropped
	// MOAC, follow EIP158 for all the blocks
...
/* func (c *Clique) FinalizeNotifications(txs []*types.Transaction) {
	log.Debugf("[consensus/clique/clique.go->FinalizeNotifications]")
	nr := networkrelay.GetInstance()
	for _, tx := range txs {
		hash := tx.Hash()
		var liveinfo vtypes.LiveInfo
		nr.NotifyScsFinalize(tx, hash, liveinfo)
	}
} */
