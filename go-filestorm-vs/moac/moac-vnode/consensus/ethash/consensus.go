...
var (
	// panguBlockReward     *big.Int = big.NewInt(2e+18)    // Block reward in sha for successfully mining a block
...
func (ethash *Ethash) verifyHeader(chain consensus.ChainReader, header, parent *types.Header, uncle bool, seal bool) error {
	...
	//MOAC 2018/03/21 Don't need to check for the following forks.
...
func CalcDifficulty(config *params.ChainConfig, time uint64, parent *types.Header) *big.Int {
	...
	return calcDifficultyPangu(time, parent)
	// switch {
	// case config.IsByzantium(next):
	// 	return calcDifficultyByzantium(time, parent)
	// case config.IsPangu(next):
	// 	return calcDifficultyPangu(time, parent)
	// default:
...
// calcDifficultyPangu
func calcDifficultyPangu(time uint64, parent *types.Header) *big.Int {
	// algorithm:
	// diff = (parent_diff +
...
func (ethash *Ethash) Finalize(chain consensus.ChainReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header, receipts []*types.Receipt, liveFlag bool) (*types.Block, error) {
	//log.Infof("[consensus/ethash/consensus.go->Finalize]")
...
/* func (ethash *Ethash) FinalizeNotifications(txs []*types.Transaction, blknum *big.Int) {
	log.Debugf("[consensus/ethash/consensus.go->FinalizeNotifications]")
	nr := networkrelay.GetInstance()
	for _, tx := range txs {
		hash := tx.Hash()
		liveinfo := vtypes.LiveInfo{CurrentBlockNum: blknum}
