...
import (
	...
	"github.com/filestorm/go-filestorm/moac/moac-vnode"
...
type SimulatedBackend struct {
	database   mcdb.Database    // In memory database to store our testing data
	blockchain *core.BlockChain // MoacNode blockchain to handle the consensus
...
func NewSimulatedBackend(alloc core.GenesisAlloc) *SimulatedBackend {
	database, _ := mcdb.NewMemDatabase()
...
func (b *SimulatedBackend) Commit() {
	...
	liveFlag := false
	if _, err := b.blockchain.InsertChain([]*types.Block{b.pendingBlock}, liveFlag); err != nil {
...
func (b *SimulatedBackend) callContract(ctx context.Context, call moaccore.CallMsg, block *types.Block, statedb *state.StateDB) ([]byte, *big.Int, error) {
	// log.Info("[accounts/abi/bind/backends/simulated.go->callContract]")
...
// Update:
// The PanguSigner requires the chainid to be included
func (b *SimulatedBackend) SendTransaction(ctx context.Context, tx *types.Transaction) error {
	log.Debugf("[accounts/abi/bind/backends/simulated.go->SimulatedBackend.SendTransaction]")
	b.mu.Lock()
	defer b.mu.Unlock()

	//Need ChainId to init the Signer
	// ErrInvalidChainId = errors.New("invalid chain id for signer")
	// if ( tx.ChainId() == nil)
	// return errors.New("invalid chain id for signer")

	sender, err := types.Sender(types.NewPanguSigner(tx.ChainId()), tx)
	if err != nil {
...
//Merge on 2018/0411
func (m callmsg) GasLimit() *big.Int { return m.CallMsg.GasLimit }
