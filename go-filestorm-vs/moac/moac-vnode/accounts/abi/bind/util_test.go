...
import (
	...
	"github.com/filestorm/go-filestorm/moac/moac-vnode/accounts/abi/bind/backends"
...
func TestWaitDeployed(t *testing.T) {
	for name, test := range waitDeployedTests {
		...
		tx := types.NewContractCreation(0, big.NewInt(0), test.gas, big.NewInt(1), 0, 0, common.FromHex(test.code), nil)
		tx, _ = types.SignTx(tx, types.PanguSigner{}, testKey)
