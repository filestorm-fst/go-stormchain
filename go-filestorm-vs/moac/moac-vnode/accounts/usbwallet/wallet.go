...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the MoacNode
...
type wallet struct {
	...
	deriveChain    moaccore.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived MoacNode account
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain moaccore.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
...
func (w *wallet) SignTx(account accounts.Account, tx *types.Transaction, chainID *big.Int) (*types.Transaction, error) {
	// log.Info("[accounts/usbwallet/wallet.go->wallet.SignTx]")
