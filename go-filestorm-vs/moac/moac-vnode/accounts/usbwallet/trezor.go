...
import (
	...
	"github.com/filestorm/go-filestorm/moac/moac-vnode/accounts/usbwallet/internal/trezor"
...
// or whether the MoacNode app was not started on it.
...
func (w *trezorDriver) SignTx(path accounts.DerivationPath, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	// log.Info("[accounts/usbwallet/trezor.go->trezorDriver.SignTx]")
	if w.device == nil {
...
// MoacNode address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.MoacNodeAddress)
	if _, err := w.trezorExchange(&trezor.MoacNodeGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.MoacNodeTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.MoacNodeTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
