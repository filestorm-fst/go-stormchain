...
// https://raw.githubusercontent.com/LedgerHQ/blue-app-mc/master/doc/ethapp.asc
...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and MoacNode address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an MoacNode transaction after having the user validate the parameters
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "MoacNode app in browser mode", w.failure
	}
	if w.offline() {
		return "MoacNode app offline", w.failure
	}
	return fmt.Sprintf("MoacNode app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}

// offline returns whether the wallet and the MoacNode app is offline or not.
//
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// MoacNode app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the MoacNode app's version, will fail prior to v1.0.2
...
// and returning the MoacNode address located on that derivation path.
...
//
// Note, if the version of the MoacNode application running on the Ledger wallet is
// too old to sign EIP-155 transactions, but such is requested nonetheless, an error
// will be returned opposed to silently signing in Pangu mode.
func (w *ledgerDriver) SignTx(path accounts.DerivationPath, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	// log.Info("[accounts/usbwallet/ledger.go->ledgerDriver.SignTx]")
	// If the MoacNode app doesn't run, abort
	if w.offline() {
...
// ledgerVersion retrieves the current version of the MoacNode wallet app running
...
// ledgerDerive retrieves the currently active MoacNode address from a Ledger
...
//   MoacNode address length | 1 byte
//   MoacNode address        | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the MoacNode hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an MoacNode address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// ONLY USE EIP155 signature, or PanGu sign
	// if chainID == nil {
	...
	if txrlp, err = rlp.EncodeToBytes([]interface{}{tx.Nonce(), tx.GasPrice(), tx.GasLimit(), tx.To(), tx.Value(), tx.Data(), chainID, big.NewInt(0), big.NewInt(0)}); err != nil {
		return common.Address{}, nil, err
	}
	// }
	...
	// Extract the MoacNode signature and do a sanity validation
	if len(reply) != 65 {
	...
	// if chainID == nil {
	// 	signer = new(types.PanguSigner)
	// } else {
	// signer = types.NewEIP155Signer(chainID)
	signer = types.NewPanguSigner(chainID)
