...
// faucet is a MOAC faucet backed by a light client.
package main
...
var (
	...
	ethPortFlag = flag.Int("ethport", 30313, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 1, "Network ID to use for the MoacNode protocol")
	statsFlag   = flag.String("mcstats", "", "Moacstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of MOACs to pay out per user request")
	...
	logFlag = flag.Int("loglevel", 3, "Log level to use for MoacNode and the faucet")
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s MOACs", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
...
type request struct {
	...
	Account  common.Address     `json:"account"`  // MoacNode address being funded
	Time     time.Time          `json:"time"`     // Timestamp when te request was accepted
	Tx       *types.Transaction `json:"tx"`       // Transaction funding the account
}

// faucet represents a crypto faucet backed by an MoacNode light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // MoacNode protocol stack
	client *mcclient.Client    // Client connection to the MoacNode chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the MoacNode light client protocol
	...
	// Assemble the mcstats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightMoacNode
			ctx.Service(&serv)
			return mcstats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
...
// close terminates the MoacNode connection and tears down the faucet.
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		log.Infof("Faucet funds requested gist=%v tier=%v", msg.URL, msg.Tier)
		...
		// Iterate over all the files and look for MoacNode addresses
		...
		for _, file := range gist.Files {
			...
			if len(content) == 2+common.AddressLength*2 {
				...
				address = common.MoacToAddress(content)
			}
		}
		if address == (common.Address{}) {
			websocket.JSON.Send(conn, map[string]string{"error": "No MoacNode address found to fund"})
			continue
		}
		...
		if timeout = f.timeouts[gist.Owner.Login]; time.Now().After(timeout) {
			...
			tx := types.NewTransaction(f.nonce+uint64(len(f.reqs)), address, amount, big.NewInt(21000), f.price, 0, nil, nil)
