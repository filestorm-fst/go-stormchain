...
var (
	walletCommand = cli.Command{
		Name:      "wallet",
		Usage:     "Manage MoacNode presale wallets",
		ArgsUsage: "",
		Category:  "ACCOUNT COMMANDS",
		Description: `
    moac wallet import /path/to/my/presale.wallet
		...
		Subcommands: []cli.Command{
			{
				...
				Name:      "import",
				Usage:     "Import MOAC presale wallet",
				ArgsUsage: "<keyFile>",
				...
				Description: `
	moac wallet [options] /path/to/my/presale.wallet
...
between MOAC nodes by simply copying.
...
For non-interactive use the passphrase can be specified with the --password flag:
    ...
    moac account update [options] <address>
...
For non-interactive use the passphrase can be specified with the -password flag:
    ...
    moac account import [options] <keyfile>

Note:
As you can directly copy your encrypted accounts to another MOAC instance,
...
func accountList(ctx *cli.Context) error {
	stack, _ := makeNodeFromConfig(ctx)
	var index int
...
func accountCreate(ctx *cli.Context) error {
	stack, _ := makeNodeFromConfig(ctx)
	password := getPassPhrase("A new MOAC account will be generated and locked with a password. Please give a password and save it in a safe place. Do not forget this password.", true, 0, utils.MakePasswordList(ctx))
