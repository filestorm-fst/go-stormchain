...
func (w *wizard) manageServers() {
	...
	if choice <= len(w.conf.Servers) {
		...
		log.Infof("Disconnected existing server server=%v", server)
...
func (w *wizard) manageComponents() {
	...
	if choice <= len(serviceHosts) {
		...
		log.Infof("Torn down existing component server=%v service=%v", server, service)
		return
	}
...
func (w *wizard) deployComponent() {
	...
	fmt.Println(" 1. Mcstats  - Network monitoring tool")
