...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.mcstats == "" {
		log.Error("No mcstats server configured")
		return
	}
	...
	if err != nil {
		if boot {
			infos = &nodeInfos{portFull: 30333, peersTotal: 512, peersLight: 256}
		} else {
			infos = &nodeInfos{portFull: 30333, peersTotal: 50, peersLight: 0, gasTarget: 4.7, gasPrice: 18}
		}
	}
	...
	if infos.mcstats == "" {
		...
		infos.mcstats = w.readString() + ":" + w.conf.mcstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.mcstats)
		infos.mcstats = w.readDefaultString(infos.mcstats) + ":" + w.conf.mcstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.genesis.Config.Ethash != nil {
			// Ethash based miners only need an moacbase to mine against
			fmt.Println()
			if infos.moacbase == "" {
				fmt.Printf("What address should the miner user?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.moacbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner user? (default = %s)\n", infos.moacbase)
				infos.moacbase = w.readDefaultAddress(common.HexToAddress(infos.moacbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootFull, w.conf.bootLight, infos); err != nil {
		log.Error("Failed to deploy MoacNode node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
