...
RUN \
  apk add --update git go make gcc musl-dev ca-certificates linux-headers                             && \
	...
	(cd $GOPATH/src/github.com/ethereum && git clone --depth=1 https://github.com/filestorm/go-filestorm/moac/moac-vnode) && \
  go build -v github.com/filestorm/go-filestorm/moac/moac-vnode/cmd/faucet                                              && \
...
CMD [ \
	"/faucet", "--genesis", "/genesis.json", "--network", "{{.NetworkID}}", "--bootnodes", "{{.Bootnodes}}", "--mcstats", "{{.Mcstats}}", "--ethport", "{{.EthPort}}", \
...
func deployFaucet(client *sshClient, network string, bootnodes []string, config *faucetInfos) ([]byte, error) {
	...
	template.Must(template.New("").Parse(faucetComposefile)).Execute(composefile, map[string]interface{}{
		...
		"ApiPort":       config.port,
		"EthPort":       config.node.portFull,
		"EthName":       config.node.mcstats[:strings.Index(config.node.mcstats, ":")],
		"GitHubUser":    config.githubUser,
		"GitHubToken":   config.githubToken,
		"CaptchaToken":  config.captchaToken,
		"CaptchaSecret": config.captchaSecret,
...
func (info *faucetInfos) String() string {
	return fmt.Sprintf("host=%s, api=%d, mc=%d, amount=%d, minutes=%d, tiers=%d, github=%s, captcha=%v, mcstats=%s", info.host, info.port, info.node.portFull, info.amount, info.minutes, info.tiers, info.githubUser, info.captchaToken != "", info.node.mcstats)
}
...
func checkFaucet(client *sshClient, network string) (*faucetInfos, error) {
	...
	return &faucetInfos{
		node: &nodeInfos{
			...
			mcstats:  infos.envvars["ETH_NAME"],
			keyJSON:  keyJSON,
			keyPass:  keyPass,
		},
		host:          host,
