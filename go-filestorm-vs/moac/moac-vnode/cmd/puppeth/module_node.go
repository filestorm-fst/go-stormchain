...
// nodeDockerfile is the Dockerfile required to run an MoacNode node.
var nodeDockerfile = `
...
{{end}}
RUN \
  echo 'moac init /genesis.json' > moac.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.moac/keystore/ && cp /signer.json /root/.moac/keystore/' >> moac.sh && \{{end}}
	echo $'moac --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --maxpeers {{.Peers}} {{.LightFlag}} --mcstats \'{{.Mcstats}}\' {{if .BootV4}}--bootnodesv4 {{.BootV4}}{{end}} {{if .BootV5}}--bootnodesv5 {{.BootV5}}{{end}} {{if .Moacbase}}--moacbase {{.Moacbase}} --mine{{end}}{{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --targetgaslimit {{.GasTarget}} --gasprice {{.GasPrice}}' >> moac.sh

ENTRYPOINT ["/bin/sh", "moac.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an MoacNode node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
...
func deployNode(client *sshClient, network string, bootv4, bootv5 []string, config *nodeInfos) ([]byte, error) {
	...
	template.Must(template.New("").Parse(nodeComposefile)).Execute(composefile, map[string]interface{}{
		...
		"Mcstats":    config.mcstats[:strings.Index(config.mcstats, ":")],
		"Moacbase":   config.moacbase,
...
func (info *nodeInfos) String() string {
	...
	return fmt.Sprintf("port=%d%s, datadir=%s, peers=%d, lights=%d, mcstats=%s, gastarget=%0.3f MGas, gasprice=%0.3f GWei",
		info.portFull, discv5, info.datadir, info.peersTotal, info.peersLight, info.mcstats, info.gasTarget, info.gasPrice)
}
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 moac --exec admin.nodeInfo.id attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.moac"],
		portFull:   infos.portmap[infos.envvars["FULL_PORT"]],
		portLight:  infos.portmap[infos.envvars["LIGHT_PORT"]],
		peersTotal: totalPeers,
		peersLight: lightPeers,
		mcstats:    infos.envvars["STATS_NAME"],
		moacbase:   infos.envvars["MINER_NAME"],
		keyJSON:    keyJSON,
		keyPass:    keyPass,
		gasTarget:  gasTarget,
		gasPrice:   gasPrice,
