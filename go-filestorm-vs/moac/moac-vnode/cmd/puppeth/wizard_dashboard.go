...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"mcstats", "explorer", "wallet", "faucet"} {
	...
	// If we have mcstats running, ask whether to make the secret public or not
	var mcstats bool
	if w.conf.mcstats != "" {
		fmt.Println()
		fmt.Println("Include mcstats secret on dashboard (y/n)? (default = yes)")
		mcstats = w.readDefaultString("y") == "y"
	}
	// Try to deploy the dashboard container on the host
	if out, err := deployDashboard(client, w.network, infos.port, infos.host, listing, &w.conf, mcstats); err != nil {
