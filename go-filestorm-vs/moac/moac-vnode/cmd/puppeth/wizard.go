...
type config struct {
	...
	mcstats   string        // Mcstats settings to cache for node deploys
...
type wizard struct {
	...
	services map[string][]string   // MoacNode services known to be running on servers
...
// converts it to an MoacNode address. If an empty line is entered, the default
// value is returned.
