...
func (w *wizard) networkStats(tips bool) {
	...
	for server, pubkey := range w.conf.Servers {
		...
		logger.Debug("Checking for mcstats availability")
		if infos, err := checkMcstats(client, w.network); err != nil {
			if err != ErrServiceUnknown {
				services["mcstats"] = err.Error()
			}
		} else {
			services["mcstats"] = infos.String()
			protips.mcstats = infos.config
		}
	...
	if protips.mcstats != "" {
		w.conf.mcstats = protips.mcstats
	}
...
func (p *protips) print(network string) {
	...
	fullinit, lightinit := "", ""
	if p.genesis != "" {
		fullinit = fmt.Sprintf("moac --datadir=$HOME/.%s init %s.json && ", network, network)
		lightinit = fmt.Sprintf("moac --datadir=$HOME/.%s --light init %s.json && ", network, network)
	}
	// If an mcstats server is available, add the mcstats flag
	statsflag := ""
	if p.mcstats != "" {
		if strings.Contains(p.mcstats, " ") {
			statsflag = fmt.Sprintf(` --mcstats="yournode:%s"`, p.mcstats)
		} else {
			statsflag = fmt.Sprintf(` --mcstats=yournode:%s`, p.mcstats)
		}
	}
