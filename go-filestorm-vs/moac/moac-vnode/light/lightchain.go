...
// available in the database. It initialises the default MoacNode header
// validator.
func NewLightChain(odr OdrBackend, config *params.ChainConfig, engine consensus.Engine) (*LightChain, error) {
	...
	if bc.genesisBlock.Hash() == params.MainnetGenesisHash {
		...
		log.Info("Added trusted CHT for MOAC mainnet")
	}
	if bc.genesisBlock.Hash() == params.TestnetGenesisHash {
		...
		log.Info("Added trusted CHT for MOAC testnet")
	}
...
func (self *LightChain) loadLastState() error {
	...
	log.Infof("Loaded most recent local header number=%v hash=%v td=%v", header.Number, header.Hash(), headerTd)
