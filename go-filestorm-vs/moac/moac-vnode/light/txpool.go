...
func NewTxPool(config *params.ChainConfig, chain *LightChain, relay TxRelayBackend) *TxPool {
	pool := &TxPool{
		config:      config,
		signer:      types.NewPanguSigner(config.ChainId),
...
func (pool *TxPool) setNewHead(head *types.Header) {
	...
	pool.pangu = pool.config.IsPangu(head.Number)
...
func (pool *TxPool) SubscribeTxShardJoinEvent(ch chan<- core.TxShardJoinEvent) event.Subscription {
	return pool.scope.Track(pool.txFeed.Subscribe(ch))
}
...
func (pool *TxPool) validateTx(ctx context.Context, tx *types.Transaction) error {
	...
	if from, err = types.Sender(pool.signer, tx); err != nil {
		log.Info("Invalid sender in validateTx")
		fmt.Println(err)
	...
	if n := currentState.GetNonce(from); n > tx.Nonce() {
		log.Info("expected nonce", "nonce", n)
	...
	if header.GasLimit.Cmp(tx.GasLimit()) < 0 {
		log.Debugf("tx.Gas %v, header.GasLimit %v", tx.GasLimit(), header.GasLimit)
	...
	if b := currentState.GetBalance(from); b.Cmp(tx.Cost()) < 0 {
		log.Info("Balance", b.String(), " is smaller than", tx.Cost().String())
		return core.ErrInsufficientFunds
	}

	// Should supply enough intrinsic gas
	if tx.GasLimit().Cmp(core.IntrinsicGas(tx.Data(), tx.To() == nil, pool.pangu)) < 0 {
