...
type testOdr struct {
	OdrBackend
	sdb, ldb mcdb.Database
	disable  bool
}

func (odr *testOdr) Database() mcdb.Database {
	return odr.ldb
}
...
type odrTestFn func(ctx context.Context, db mcdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error)

func TestOdrGetBlockLes1(t *testing.T) { testChainOdr(t, 1, odrGetBlock) }

func odrGetBlock(ctx context.Context, db mcdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrGetReceipts(ctx context.Context, db mcdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrAccounts(ctx context.Context, db mcdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrContractCall(ctx context.Context, db mcdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
	log.Info("[light/odr_test.go->odrContractCall in]")
...
func testChainGen(i int, block *core.BlockGen) {
	signer := types.PanguSigner{}
	switch i {
	case 0:
		// In block 1, the test bank sends account #1 some ether.
		tx, _ := types.SignTx(types.NewTransaction(block.TxNonce(testBankAddress), acc1Addr, big.NewInt(10000), bigTxGas, nil, nil, nil), signer, testBankKey)
		block.AddTx(tx)
	case 1:
		...
		tx2, _ := types.SignTx(types.NewTransaction(nonce, acc2Addr, big.NewInt(1000), bigTxGas, nil, nil, nil), signer, acc1Key)
		nonce++
	...
	case 2:
		...
		tx, _ := types.SignTx(types.NewTransaction(block.TxNonce(testBankAddress), testContractAddr, big.NewInt(0), big.NewInt(100000), nil, nil, data), signer, testBankKey)
		block.AddTx(tx)
	case 3:
...
func testChainOdr(t *testing.T, protocol int, fn odrTestFn) {
	var (
		sdb, _  = mcdb.NewMemDatabase()
		ldb, _  = mcdb.NewMemDatabase()
	...
	if _, err := blockchain.InsertChain(gchain,false); err != nil {
		t.Fatal(err)
	}
