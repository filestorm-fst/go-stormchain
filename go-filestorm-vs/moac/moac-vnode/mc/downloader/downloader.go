...
import (
	...
	moaccore "github.com/filestorm/go-filestorm/moac/moac-vnode"
...
var (
	...
	maxQueuedHeaders  = 32 * 1024 // [mc/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	// Channels
	headerCh      chan dataPack        // [mc/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [mc/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [mc/63] Channel receiving inbound receipts
	bodyWakeCh    chan bool            // [mc/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [mc/63] Channel to signal the receipt fetcher of new tasks
	headerProcCh  chan []*types.Header // [mc/62] Channel to feed the header processor new tasks

	// for stateFetcher
	stateSyncStart chan *stateSync
	trackStateReq  chan *stateReq
	stateCh        chan dataPack // [mc/63] Channel receiving inbound node state data
...
func New(mode SyncMode, stateDb mcdb.Database, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
...
// these are zero.
func (d *Downloader) Progress() moaccore.SyncProgress {
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	for len(results) != 0 {
		...
		//log.Infof("Downloader importBlockResults liveFlag %v", liveFlag)
