...
type peerDropFn func(id string, subnet string)
...
type Fetcher struct {
	...
	fetchingHook       func([]common.Hash)     // Method to call upon starting a block (mc/61) or header (mc/62) fetch
	completingHook     func([]common.Hash)     // Method to call upon starting a block body fetch (mc/62)
	importedHook       func(*types.Block)      // Method to call upon successful block import (both mc/61 and mc/62)
}
			...
			for _, header := range task.headers {
				...
				if announce := f.fetching[hash]; announce != nil && f.fetched[hash] == nil && f.completing[hash] == nil && f.queued[hash] == nil {
					...
					if header.Number.Uint64() != announce.number {
						...
						f.dropPeer(announce.origin, "mainnet")
						f.forgetHash(hash)
						continue
					}
