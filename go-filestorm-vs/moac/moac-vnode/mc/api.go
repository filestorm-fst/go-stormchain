...
import (
	...
	"github.com/filestorm/go-filestorm/moac/moac-vnode/mc/tracers"
...
func (api *PublicMoacAPI) Moacbase() (common.Address, error) {
	//log.Info("[mc/api.go->PublicMoacAPI.Moacbase]")
...
func (api *PublicMoacAPI) Coinbase() (common.Address, error) {
	//log.Info("[mc/api.go->PublicMoacAPI.Coinbase]")
	// coinbaseAddress, err := api.Moacbase()
	// return coinbaseAddress.GetBase58Str(), err
	return api.Moacbase()
}
...
// ListSubnetP2P requests disconnecting this node to subnet's dedicated p2p network
func (api *PrivateAdminAPI) ListSubnetP2P() ([]string, error) {
...
func (api *PrivateAdminAPI) ImportChain(file string) (bool, error) {
	...
	for batch := 0; ; batch++ {
		...
		liveFlag := false
		//log.Infof("PrivateAdminAPI liveFlag %v", liveFlag)
...
func (api *PublicDebugAPI) DumpBlock(blockNr rpc.BlockNumber) (state.Dump, error) {
	...
	stateDb, err := api.mc.BlockChain().StateAt(block.Root())
	if err != nil {
...
func (api *PrivateDebugAPI) computeTxEnv(blockHash common.Hash, txIndex int) (core.Message, vm.Context, *state.StateDB, error) {
	...
	statedb, err := api.mc.BlockChain().StateAt(parent.Root())
	if err != nil {
