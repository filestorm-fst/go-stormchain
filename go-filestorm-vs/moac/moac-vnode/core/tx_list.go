...
func (m *txSortedMap) Ready(start uint64) types.Transactions {
	...
	// log.Info("txlist.checkready", "cur txlist", m.index)
...
//
// If the new transaction is accepted into the list, the lists' cost and gasRemaining
...
func (l *txList) Add(tx *types.Transaction, priceBump uint64) (bool, *types.Transaction) {
	...
	if gasLimit := tx.GasLimit(); l.gascap.Cmp(gasLimit) < 0 {
		l.gascap = gasLimit
	}
	return true, old
}
...
// Filter removes all transactions from the list with a cost or gasRemaining limit higher
...
func (l *txList) Filter(costLimit, gasLimit *big.Int) (types.Transactions, types.Transactions) {
	...
	removed := l.txs.Filter(func(tx *types.Transaction) bool {
		return tx.Cost().Cmp(costLimit) > 0 || tx.GasLimit().Cmp(gasLimit) > 0
	})
