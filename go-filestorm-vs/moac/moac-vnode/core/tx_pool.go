...
import (
	...
	"github.com/filestorm/go-filestorm/moac/moac-vnode/networkrelay"
...
const (
	...
	// special tx use this gas price
	minSpecGasPrice = uint64(params.Xiao)
)
...
var (
	...
	// ErrMismatchNetworkid is returned if the transaction has a different network id
	// form the network.
	ErrMismatchNetworkid = errors.New("Mismatch networkid")

	// ErrNonceTooLow is returned if the nonce of a transaction is lower than the
	// one present in the local chain.
	ErrNonceTooLow = errors.New("nonce too low")

	// ErrUnderpriced is returned if a transaction's gasRemaining price is below the minimum
	...
	ErrInsufficientFunds = errors.New("insufficient funds for gasRemaining * price + value")

	// ErrIntrinsicGas is returned if the transaction is specified to use less gasRemaining
	// than required to start the invocation.
	ErrIntrinsicGas = errors.New("intrinsic gasRemaining too low")

	// ErrGasLimit is returned if a transaction's requested gasRemaining limit exceeds the
	// maximum allowance of the current block.
	ErrGasLimit = errors.New("exceeds block gasRemaining limit")
...
type TxPoolConfig struct {
	...
	PriceLimit uint64 // Minimum gasRemaining price to enforce for acceptance into the pool
...
// pool.
// 2018/03/17 change the GlobalSlots from 4096 to 40960
...
type TxPool struct {
	...
	muDC        sync.RWMutex
	directCalls map[common.Address]*txList

	wg sync.WaitGroup // for shutdown sync

	pangu bool

	KnownDirectTxs *set.Set
}

// NewTxPool creates a new transaction pool to gather, sort and filter inbound
// trnsactions from the network.
func NewTxPool(config TxPoolConfig, chainconfig *params.ChainConfig, chain blockChain) *TxPool {
	// Sanitize the input to ensure no vulnerable gasRemaning prices are set
	config = (&config).sanitize()
	fmt.Printf("Allocate new TXPOOL with id %v\n", chainconfig.ChainId)
	...
	pool := &TxPool{
		...
		directCalls:    make(map[common.Address]*txList),
		all:            make(map[common.Hash]*types.Transaction),
		chainHeadCh:    make(chan ChainHeadEvent, chainHeadChanSize),
		gasPrice:       new(big.Int).SetUint64(config.PriceLimit),
		KnownDirectTxs: set.New(),
	}
...
func (pool *TxPool) loop() {
	...
	for {
		...
		case ev := <-pool.chainHeadCh:
			if ev.Block != nil {
				pool.mu.Lock()
				if pool.chainconfig.IsPangu(ev.Block.Number()) {
					pool.pangu = true
				}
		...
		case <-report.C:
			...
			//log.Info("Txpool.loop", "report", pending)
			pool.mu.RUnlock()
		...
		case <-evict.C:
			pool.mu.Lock()
			//log.Info("Txpool.loop", "evict", pool.queue)
...
func (pool *TxPool) reset(oldHead, newHead *types.Header) {
	//clean up directCallList
	pool.initDirectCallList()
...
//func (pool *TxPool) SubscribeTxShardJoinEvent(ch chan<- TxShardJoinEvent) event.Subscription {
//	return pool.scope.Track(pool.txFeed.Subscribe(ch))
//}

// GasPrice returns the current gasRemaining price enforced by the transaction pool.
...
func (pool *TxPool) SetGasPrice(price *big.Int) {
	...
	log.Infof("Transaction pool price threshold updated price=%v", price)
}
...
// PendingByRole retrieves all currently processable transactions, groupped by scs
// role origin account and sorted by nonce. The returned transaction set is a copy
// and can be freely modified by calling code.
func (pool *TxPool) PendingByRole() (map[common.Address]types.Transactions, map[common.Address]types.Transactions) {
	pool.mu.Lock()
	defer pool.mu.Unlock()
...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	// log.Info("validateTx", "cost:", tx.Cost())

	// Heuristic limit, reject transactions over 32KB to prevent DOS attacks
	if tx.Size() > 32*1024 {
		log.Info("tx size too large")
	...
	if pool.currentMaxGas.Cmp(tx.GasLimit()) < 0 {
		log.Debugf("tx.Gas %v, pool.currentMaxGas %v", tx.GasLimit(), pool.currentMaxGas)
		return ErrGasLimit
	}
	// Make sure the transaction is signed properly
	from, err := types.Sender(pool.signer, tx)
	if err != nil {
		log.Info("InvalidSender signer")
	...
	if pool.currentState.GetNonce(from) > tx.Nonce() {
		log.Debugf("error too low nonce %v>%v, from: %v", pool.currentState.GetNonce(from), tx.Nonce(), from.String())
...
func (pool *TxPool) AddSysCntCall(tx *types.Transaction) error {
	//log.Info("txpool.addSysCntCall")

	return pool.addTx(tx, true)
}

func (pool *TxPool) AddShardJoinCall(tx *types.Transaction) error {
	//log.Info("txpool.AddShardJoinCall")
	//update nonce to date
	return pool.addTx(tx, true)
}
...
func (pool *TxPool) add(tx *types.Transaction, local bool) (bool, error) {
	log.Debugf("[core/tx_pool.go->TxPool.add] tx=%v local=%v", tx.String(), local)
	// If the transaction is already known, discard it
	// log.Debugf("[core/tx_pool.go->TxPool.add before hash] %v", tx.Value())
	hash := tx.Hash()
	if pool.all[hash] != nil {
		log.Debug("Discarding already known transaction", "hash", hash)
		return false, fmt.Errorf("known transaction: %x", hash)
	}
	log.Debugf("[core/tx_pool.go->TxPool.add] %v", tx.Value())
	...
	if list := pool.pending[from]; list != nil && list.Overlaps(tx) {
		...
		//replacement for pending should notify other nodes.
	...
	// log.Info("txpool.add", "pool.pending", pool.pending)
	return replace, nil
}
...
func (pool *TxPool) MarkDirectTx(hash common.Hash) {
	// If we reached the memory allowance, drop a previously known transaction hash
	for pool.KnownDirectTxs.Size() >= maxKnownTxs {
		pool.KnownDirectTxs.Pop()
	}
	pool.KnownDirectTxs.Add(hash)
}

func (pool *TxPool) sendDirectCall(tx *types.Transaction) error {
	//log.Infof("sendDirectCall timeInSecond %v", timeInSecond)
	txHash := tx.Hash()
	...
	res, err := nr.VnodePushMsg(req)
	if err != nil {
		log.Errorf("sendDirectCall error %v", err)
	} else {
		tx.DirCallSent = true
	}
	log.Debugf("sendDirectCall res %v", res)
...
func (pool *TxPool) manageDirectCall(addr common.Address, tx *types.Transaction) error {
	...
	//length := directCallList.Len()

	//delay := time.Duration(0)
	//if length >= params.DirectCallLimitPerBlock {
	//	//drop tx
	//	log.Errorf("[test] DirectCall droped: from %v, #%v hex %v", addr.Hex(), length, tx.Hash().Hex())
	//	return
	//	//delay = time.Second * time.Duration((length - params.DirectCallLimitPerBlock) * 5 / params.DirectCallLimitPerBlock)
	//}
	nano := time.Now().Nanosecond()
	//go pool.sendDirectCall(tx, delay, timeInSecond)
...
func (pool *TxPool) promoteTx(addr common.Address, hash common.Hash, tx *types.Transaction) {
	//log.Info("txpool.promoteTx", "addr", addr.String())
...
func (pool *TxPool) AddLocal(tx *types.Transaction) error {
	//log.Info("[core/tx_pool.go->TxPool.AddLocal]")
...
func (pool *TxPool) AddRemotes(txs []*types.Transaction) error {
	//log.Info("txpool.addremote", "txs", txs)
...
func (pool *TxPool) addTx(tx *types.Transaction, local bool) error {
	//log.Info("[core/tx_pool.go->TxPool.addTx]")
	pool.mu.Lock()
	defer pool.mu.Unlock()
	// log.Info("[core/tx_pool.go->TxPool.addTx] 2")
...
func (pool *TxPool) addTxsLocked(txs []*types.Transaction, local bool) error {
	...
	log.Debugf("addTxsLocked txs %v", txs)
	...
	for _, tx := range txs {
		...
		if replace, err := pool.add(tx, local); err == nil {
			if !replace {
				log.Debugf("validate sender from %v", from)
				dirty[from] = struct{}{}
			}
		}
	}
	...
	if len(dirty) > 0 {
		...
		// log.Info("promote tx", "addrs", addrs)
...
func (pool *TxPool) promoteExecutables(accounts []common.Address) {
	// log.Info("txpool.promoteExecutables")
	...
	for _, addr := range accounts {
		...
		//log.Info("txpool.promoteExecutables", "account", addr)
		if list == nil {
		...
		addrnonce := pool.pendingState.GetNonce(addr)
		// log.Info("txpool.promoteExecutable", "addr nonce", addrnonce)

		for _, tx := range list.Ready(addrnonce) {
			hash := tx.Hash()
...
func (pool *TxPool) initDirectCallList() {
	pool.muDC.Lock()
	defer pool.muDC.Unlock()

	pool.directCalls = make(map[common.Address]*txList)
}

func (pool *TxPool) CleanUpDirectCalls() {
	pool.muDC.Lock()
	defer pool.muDC.Unlock()

	for _, txList := range pool.directCalls {
