...
func pricedTransaction(nonce uint64, gaslimit, gasprice *big.Int, key *ecdsa.PrivateKey) *types.Transaction {
	tx, _ := types.SignTx(types.NewTransaction(nonce, common.Address{}, big.NewInt(100), gaslimit, gasprice, 0, nil, nil), types.PanguSigner{}, key)
	return tx
}
...
func deriveSender(tx *types.Transaction) (common.Address, error) {
	return types.Sender(types.PanguSigner{}, tx)
}
...
func (c *testChain) State() (*state.StateDB, error) {
	...
	if *c.trigger {
		...
		c.statedb.SetBalance(c.address, new(big.Int).SetUint64(params.Mc))
		*c.trigger = false
	}
	return stdb, nil
}
...
func TestStateChangeDuringPoolReset(t *testing.T) {
	...
	statedb.SetBalance(address, new(big.Int).SetUint64(params.Mc))
...
func TestInvalidTransactions(t *testing.T) {
	...
	balance := new(big.Int).Add(tx.Value(), new(big.Int).Mul(tx.GasLimit(), tx.GasPrice()))
...
func TestNegativeValue(t *testing.T) {
	...
	tx, _ := types.SignTx(types.NewTransaction(0, common.Address{}, big.NewInt(-1), big.NewInt(100), big.NewInt(1), 0, nil, nil), types.NewPanguSigner(big.NewInt(1)), key)
	from, _ := deriveSender(tx)
	pool.currentState.AddBalance(from, big.NewInt(1))
...
func TestTransactionDoubleNonce(t *testing.T) {
	...
	// NewTransaction(0, common.Address{}, new(big.Int), new(big.Int), new(big.Int), 0, nil)
	tx1, _ := types.SignTx(types.NewTransaction(0, common.Address{}, big.NewInt(100), big.NewInt(100000), big.NewInt(1), 0, nil, nil), signer, key)
	tx2, _ := types.SignTx(types.NewTransaction(0, common.Address{}, big.NewInt(100), big.NewInt(1000000), big.NewInt(2), 0, nil, nil), signer, key)
	tx3, _ := types.SignTx(types.NewTransaction(0, common.Address{}, big.NewInt(100), big.NewInt(1000000), big.NewInt(1), 0, nil, nil), signer, key)
