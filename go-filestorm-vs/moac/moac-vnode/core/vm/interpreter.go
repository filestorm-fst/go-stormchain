...
func NewInterpreter(evm *EVM, cfg Config) *Interpreter {
	...
	if !cfg.JumpTable[STOP].valid {
		cfg.JumpTable = panguInstructionSet
		// switch {
		// case evm.ChainConfig().IsByzantium(evm.BlockNumber):
		// 	cfg.JumpTable = byzantiumInstructionSet
		// case evm.ChainConfig().IsPangu(evm.BlockNumber):
		// 	cfg.JumpTable = panguInstructionSet
		// default:
		// 	cfg.JumpTable = panguInstructionSet //frontierInstructionSet
		// }
	}
...
func (in *Interpreter) Run(snapshot int, contract *Contract, input []byte, precompiledContracts ContractsInterface, msgHash *common.Hash) (ret []byte, err error) {
	// log.Debugf("[core/vm/interpreter.go->Run] contract %v input %v", contract, input)
	...
	for atomic.LoadInt32(&in.evm.abort) == 0 {
		...
		if !operation.valid {
			log.Debugf("!operation.valid err %v", err)
		...
		if in.Cfg.Debug {
			in.Cfg.Tracer.CaptureState(in.evm, pc, op, contract.GasRemaining, cost, mem, stack, contract, in.evm.depth, err)
			//log.Info("log info", "log", )
		}

		// execute the operation
		//log.Info("running op", "pc", pc, "op", op)

		//statedb := in.evm.StateDB.(*state.StateDB)
		//log.Info("statedb", "statedb", string(statedb.DumpAccountStorage(SystemContractEntryAddr)))

		//stack.Print()
		res, err := operation.execute(&pc, in.evm, contract, mem, stack, precompiledContracts, msgHash)
