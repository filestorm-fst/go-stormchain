...
func testBlockChainImport(chain types.Blocks, blockchain *BlockChain) error {
	for _, block := range chain {
		...
		receipts, _, usedGas, err := blockchain.Processor().Process(block, statedb, vm.Config{}, nil, false)
		if err != nil {
...
func testReorg(t *testing.T, first, second []int, td int64, full bool) {
	...
	if full {
		bc.InsertChain(makeBlockChainWithDiff(bc.genesisBlock, first, 11), false)
		bc.InsertChain(makeBlockChainWithDiff(bc.genesisBlock, second, 22), false)
	} else {
...
func testBadHashes(t *testing.T, full bool) {
	...
	if full {
		...
		_, err = bc.InsertChain(blocks, false)
	} else {
...
func TestFastVsFullChains(t *testing.T) {
	...
	var (
		...
		signer  = types.NewPanguSigner(gspec.Config.ChainId)
	)
	blocks, receipts := GenerateChain(gspec.Config, genesis, gendb, 1024, func(i int, block *BlockGen) {
		...
		if i%3 == 2 {
			for j := 0; j < i%4+1; j++ {
				tx, err := types.SignTx(types.NewTransaction(block.TxNonce(address), common.Address{0x00}, big.NewInt(1000), bigTxGas, nil, 0, nil, nil), signer, key)
				if err != nil {
					panic(err)
				}
				block.AddTx(tx)
	...
	archiveDb, _ := mcdb.NewMemDatabase()
...
func TestChainTxReorgs(t *testing.T) {
	...
	postponed, _ := types.SignTx(types.NewTransaction(0, addr1, big.NewInt(1000), bigTxGas, nil, 0, nil, nil), signer, key1)
	swapped, _ := types.SignTx(types.NewTransaction(1, addr1, big.NewInt(1000), bigTxGas, nil, 0, nil, nil), signer, key1)
	...
	chain, _ := GenerateChain(gspec.Config, genesis, db, 3, func(i int, gen *BlockGen) {
		switch i {
		case 0:
			pastDrop, _ = types.SignTx(types.NewTransaction(gen.TxNonce(addr2), addr2, big.NewInt(1000), bigTxGas, nil, 0, nil, nil), signer, key2)

			gen.AddTx(pastDrop)  // This transaction will be dropped in the fork from below the split point
			gen.AddTx(postponed) // This transaction will be postponed till block #3 in the fork

		case 2:
			freshDrop, _ = types.SignTx(types.NewTransaction(gen.TxNonce(addr2), addr2, big.NewInt(1000), bigTxGas, nil, 0, nil, nil), signer, key2)
	...
	chain, _ = GenerateChain(gspec.Config, genesis, db, 5, func(i int, gen *BlockGen) {
		switch i {
		case 0:
			pastAdd, _ = types.SignTx(types.NewTransaction(gen.TxNonce(addr3), addr3, big.NewInt(1000), bigTxGas, nil, 0, nil, nil), signer, key3)
		...
		case 2:
			...
			freshAdd, _ = types.SignTx(types.NewTransaction(gen.TxNonce(addr3), addr3, big.NewInt(1000), bigTxGas, nil, 0, nil, nil), signer, key3)
			gen.AddTx(freshAdd) // This transaction will be added exactly at reorg time

		case 3:
			futureAdd, _ = types.SignTx(types.NewTransaction(gen.TxNonce(addr3), addr3, big.NewInt(1000), bigTxGas, nil, 0, nil, nil), signer, key3)
...
func TestLogReorgs(t *testing.T) {
	...
	chain, _ := GenerateChain(params.TestChainConfig, genesis, db, 2, func(i int, gen *BlockGen) {
		if i == 1 {
			tx, err := types.SignTx(types.NewContractCreation(gen.TxNonce(addr1), new(big.Int), big.NewInt(1000000), new(big.Int), 0, code), signer, key1)
			if err != nil {
...
func TestReorgSideEvent(t *testing.T) {
	...
	replacementBlocks, _ := GenerateChain(gspec.Config, genesis, db, 4, func(i int, gen *BlockGen) {
		tx, err := types.SignTx(types.NewContractCreation(gen.TxNonce(addr1), new(big.Int), big.NewInt(1000000), new(big.Int), 0, nil), signer, key1)
		if i == 2 {
			gen.OffsetTime(-9)
		}
		if err != nil {
...
func TestEIP155Transition(t *testing.T) {
	...
	var (
		...
		gspec      = &Genesis{
			Config: &params.ChainConfig{ChainId: big.NewInt(1), PanguBlock: big.NewInt(1)},
	...
	blocks, _ := GenerateChain(gspec.Config, genesis, db, 4, func(i int, block *BlockGen) {
		var (
			...
			basicTx = func(signer types.Signer) (*types.Transaction, error) {
				return types.SignTx(types.NewTransaction(block.TxNonce(address), common.Address{}, new(big.Int), big.NewInt(21000), new(big.Int), 0, nil, nil), signer, key)
			}
		)
		switch i {
		case 0:
			tx, err = basicTx(types.PanguSigner{})
			if err != nil {
				t.Fatal(err)
			}
			block.AddTx(tx)
		case 2:
			...
			tx, err = basicTx(types.NewPanguSigner(gspec.Config.ChainId))
			if err != nil {
				t.Fatal(err)
			}
			block.AddTx(tx)
	...
	config := &params.ChainConfig{ChainId: big.NewInt(2), PanguBlock: new(big.Int)}
	blocks, _ = GenerateChain(config, blocks[len(blocks)-1], db, 4, func(i int, block *BlockGen) {
		...
		switch i {
		case 0:
			tx, err = basicTx(types.NewPanguSigner(big.NewInt(2)))
			if err != nil {
				t.Fatal(err)
			}
			block.AddTx(tx)
...
func TestEIP161AccountRemoval(t *testing.T) {
	...
	blocks, _ := GenerateChain(gspec.Config, genesis, db, 3, func(i int, block *BlockGen) {
		...
		switch i {
		case 0:
			tx, err = types.SignTx(types.NewTransaction(block.TxNonce(address), theAddr, new(big.Int), big.NewInt(21000), new(big.Int), 0, nil, nil), signer, key)
		case 1:
			tx, err = types.SignTx(types.NewTransaction(block.TxNonce(address), theAddr, new(big.Int), big.NewInt(21000), new(big.Int), 0, nil, nil), signer, key)
		case 2:
			tx, err = types.SignTx(types.NewTransaction(block.TxNonce(address), theAddr, new(big.Int), big.NewInt(21000), new(big.Int), 0, nil, nil), signer, key)
