...
func TestLookupStorage(t *testing.T) {
	...
	tx1 := types.NewTransaction(1, common.BytesToAddress([]byte{0x11}), big.NewInt(111), big.NewInt(1111), big.NewInt(11111), 0, nil, []byte{0x11, 0x11, 0x11})
	tx2 := types.NewTransaction(2, common.BytesToAddress([]byte{0x22}), big.NewInt(222), big.NewInt(2222), big.NewInt(22222), 0, nil, []byte{0x22, 0x22, 0x22})
	tx3 := types.NewTransaction(3, common.BytesToAddress([]byte{0x33}), big.NewInt(333), big.NewInt(3333), big.NewInt(33333), 0, nil, []byte{0x33, 0x33, 0x33})
...
// Tests that sync block hash associated with a count can be stored and retrieved.
func TestSyncBlockCnt(t *testing.T) {
	db, _ := mcdb.NewMemDatabase()
	scsId := common.Hex2Bytes("0x0123456789012345678901234567890123456789")
	subchainId := common.Hex2Bytes("0x1234567890123456789012345678901234567890")
	hashin := common.HexToHash("0x0123456789012345678901234567890123456789012345678901234567890000")
	writeCount := uint64(101)
	if ret := GetSyncBlockCnt(db, scsId, subchainId, hashin); ret != writeCount {
		if err := WriteSyncBlockCnt(db, scsId, subchainId, hashin, writeCount); err != nil {
			t.Fatalf("failed to write sync block count: %v", err)
		}

		if ret = GetSyncBlockCnt(db, scsId, subchainId, hashin); ret != writeCount {
			t.Fatalf("failed to get sync block ret: %v, real: %v", ret, writeCount)
		}
	}
}
