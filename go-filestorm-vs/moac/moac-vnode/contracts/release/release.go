...
type Config struct {
	Oracle common.Address // MoacNode address of the release oracle
...
func NewReleaseService(ctx *node.ServiceContext, config Config) (node.Service, error) {
	// Retrieve the MoacNode service dependency to access the blockchain
	var apiBackend mcapi.Backend
	var mcnode *mc.MoacService
	if err := ctx.Service(&mcnode); err == nil {
		apiBackend = mcnode.ApiBackend
	} else {
		var mcnode *les.LightMoacNode
		if err := ctx.Service(&mcnode); err == nil {
			apiBackend = mcnode.ApiBackend
		} else {
			return nil, err
	...
	contract, err := NewReleaseOracle(config.Oracle, mc.NewContractBackend(apiBackend))
	if err != nil {
		return nil, err
	}
...
func (r *ReleaseService) checkVersion() {
	...
	if version.Major > r.config.Major ||
		...
		howtofix := fmt.Sprintf("Please check https://github.com/filestorm/go-filestorm/moac/moac-vnode/releases for new releases")
