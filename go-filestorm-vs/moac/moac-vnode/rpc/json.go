...
func parseRequest(incomingMsg json.RawMessage) ([]rpcRequest, bool, Error) {
	//log.Info("[rpc/json.go->parseRequest]")
	var in jsonRequest
	if err := json.Unmarshal(incomingMsg, &in); err != nil {
		return nil, false, &invalidMessageError{err.Error()}
	}
	// log.Infof("[rpc/json.go->parseRequest]: %v\n", in.Id)
	...
	if elems[0] == "eth" {
		// upon receiving eth_xxx requests, call mc service for it as well.
		log.Debugf("Change eth to mc")
		elems[0] = "mc"
	}
	//backward compatibility for client sending web3_xxx rpc requests
	if elems[0] == "web3" {
...
func parsePositionalArguments(rawArgs json.RawMessage, types []reflect.Type) ([]reflect.Value, Error) {
	...
	for i := 0; dec.More(); i++ {
		if i >= len(types) {
			return nil, &invalidParamsError{fmt.Sprintf("too many JSON arguments, want at most %d", len(types))}
		}
		argval := reflect.New(types[i])
		if err := dec.Decode(argval.Interface()); err != nil {
			return nil, &invalidParamsError{fmt.Sprintf("invalid JSON argument %d: %v", i, err)}
		}
		if argval.IsNil() && types[i].Kind() != reflect.Ptr {
			return nil, &invalidParamsError{fmt.Sprintf("missing JSON value for required argument %d", i)}
		}
...
func (c *jsonCodec) CreateErrorResponse(id interface{}, err Error) interface{} {
	//log.Info("[rpc/json.go->CreateErrorResponse]")
