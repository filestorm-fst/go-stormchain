...
import (
	"context"
	"fmt"
	"github.com/filestorm/go-filestorm/moac/moac-vnode/rpc/set.v0"
	"reflect"
	"runtime"
	"strings"
	"sync"
...
func (s *Server) RegisterName(name string, rcvr interface{}) error {
	...
	// log.Infof("Service RegisterName:%v\n", svc.name)
...
func (s *Server) ServeCodec(codec ServerCodec, options CodecOption) {
	//log.Info("[rpc/server.go->Server.ServeCodec]")
	defer codec.Close()
...
func (s *Server) ServeSingleRequest(codec ServerCodec, options CodecOption) {
	//log.Info("[rpc/server.go->Server.ServeSingleRequest]")
...
// Some issues with the send transaction call back
//
func (s *Server) handle(ctx context.Context, codec ServerCodec, req *serverRequest) (interface{}, func()) {
	...
	//Note req could be nil
	// if reply != nil {
	// 	// log.Infof("[rpc/server.go->handle: before return] %v", reply)
	// 	if len(reply) > 0 {
...
func (s *Server) readRequest(codec ServerCodec) ([]*serverRequest, bool, Error) {
	//log.Info("[rpc/server.go->Server.readRequest]")
	...
	for i, r := range reqs {
		...
		if r.isPubSub { // mc_subscribe, r.method contains the subscription method name
