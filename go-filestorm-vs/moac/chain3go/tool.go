...
func ClientDeployContract(client *mcclient.Client ,keystore string,password string, name string,uniqueId *big.Int, blockSec *big.Int, flushNumber *big.Int, initialValidators []common.Address, totalSupply *big.Int) (contract string,tx string,err error){
...
func ClientFlush(client *mcclient.Client ,keystore string,password string, contract string ,validators []common.Address,blockNumber *big.Int, blockHash string) (txHash string,err error){
	privateKey,err :=GetPrivateKey(keystore,password)
	if err != nil {
		log.Error(err.Error())
		return "",err
