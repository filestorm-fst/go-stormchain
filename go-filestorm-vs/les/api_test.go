...
import (
	...
	"github.com/filestorm/go-filestorm/p2p/simulations"
	"github.com/filestorm/go-filestorm/p2p/simulations/adapters"
...
func getHead(ctx context.Context, t *testing.T, client *rpc.Client) (uint64, common.Hash) {
	...
	if err := client.CallContext(ctx, &res, "fst_getBlockByNumber", "latest", false); err != nil {
...
func testRequest(ctx context.Context, t *testing.T, client *rpc.Client) bool {
	...
	defer cancel()
	err := client.CallContext(c, &res, "fst_getBalance", addr, "latest")
	if err != nil {
...
func newLesServerService(ctx *adapters.ServiceContext) (node.Service, error) {
	...
	filestorm, err := fst.New(ctx.NodeContext, &config)
	if err != nil {
		return nil, err
	}
	server, err := NewLesServer(filestorm, &config)
	if err != nil {
		return nil, err
	}
	filestorm.AddLesServer(server)
	return filestorm, nil
}
