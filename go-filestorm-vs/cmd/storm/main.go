...
// storm is the official command-line client for Filestorm.
package main
...
import (
	...
	"github.com/filestorm/go-filestorm/console"
...
var (
	...
	nodeFlags = []cli.Flag{
		...
		utils.FstashCacheDirFlag,
		utils.FstashCachesInMemoryFlag,
		utils.FstashCachesOnDiskFlag,
		utils.FstashDatasetDirFlag,
		utils.FstashDatasetsInMemoryFlag,
		utils.FstashDatasetsOnDiskFlag,
...
func init() {
	// Initialize the CLI app and start Storm
	app.Action = storm
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2018-2019 The go-filestorm Authors"
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	ethClient := fstclient.NewClient(rpcClient)

	// Set contract backend for filestorm service if local node
	// is serving LES requests.
	if ctx.GlobalInt(utils.LightLegacyServFlag.Name) > 0 || ctx.GlobalInt(utils.LightServeFlag.Name) > 0 {
		var ethService *fst.Filestorm
		if err := stack.Service(&ethService); err != nil {
			utils.Fatalf("Failed to retrieve filestorm service: %v", err)
		}
	...
	if ctx.GlobalString(utils.SyncModeFlag.Name) == "light" {
		var lesService *les.LightFilestorm
		if err := stack.Service(&lesService); err != nil {
			utils.Fatalf("Failed to retrieve light filestorm service: %v", err)
		}
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		// Mining only makes sense if a full Filestorm node is running
		if ctx.GlobalString(utils.SyncModeFlag.Name) == "light" {
			utils.Fatalf("Light clients do not support mining")
		}
		var filestorm *fst.Filestorm
		if err := stack.Service(&filestorm); err != nil {
			utils.Fatalf("Filestorm service not running: %v", err)
		}
		...
		filestorm.TxPool().SetGasPrice(gasprice)

		threads := ctx.GlobalInt(utils.MinerLegacyThreadsFlag.Name)
		if ctx.GlobalIsSet(utils.MinerThreadsFlag.Name) {
			threads = ctx.GlobalInt(utils.MinerThreadsFlag.Name)
		}
		if err := filestorm.StartMining(threads); err != nil {
