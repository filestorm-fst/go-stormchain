...
type config struct {
	...
	fststats  string   // Fststats settings to cache for node deploys
...
type wizard struct {
	...
	services map[string][]string   // Filestorm services known to be running on servers
...
// converts it to an Filestorm address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by fststats, which compares IPs textually, not by value.
