...
// nodeDockerfile is the Dockerfile required to run an Filestorm node.
var nodeDockerfile = `
FROM filestorm/client-go:latest
...
RUN \
  echo 'storm --cache 512 init /genesis.json' > storm.sh && \{{if .Unlock}}
	...
	echo $'exec storm --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --fststats \'{{.Fststats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Stormbase}}--miner.stormbase {{.Stormbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> storm.sh

ENTRYPOINT ["/bin/sh", "storm.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an Filestorm node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.filestorm{{if .Fstashdir}}
      - {{.Fstashdir}}:/root/.fstash{{end}}
    environment:
      - PORT={{.Port}}/tcp
      - TOTAL_PEERS={{.TotalPeers}}
      - LIGHT_PEERS={{.LightPeers}}
      - STATS_NAME={{.Fststats}}
      - MINER_NAME={{.Stormbase}}
...
// deployNode deploys a new Filestorm node container to a remote machine via SSH,
// docker and docker-compose. If an instance with the specified network name
// already exists there, it will be overwritten!
func deployNode(client *sshClient, network string, bootnodes []string, config *nodeInfos, nocache bool) ([]byte, error) {
	kind := "sealnode"
	if config.keyJSON == "" && config.stormbase == "" {
		kind = "bootnode"
...
func (info *nodeInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Fststats username":        info.fststats,
	}
	if info.gasTarget > 0 {
		...
		if info.stormbase != "" {
			...
			report["Miner account"] = info.stormbase
		}
		if info.keyJSON != "" {
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 storm --exec admin.nodeInfo.enode --cache=16 attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.filestorm"],
		fstashdir:  infos.volumes["/root/.fstash"],
		port:       port,
		peersTotal: totalPeers,
		peersLight: lightPeers,
		fststats:   infos.envvars["STATS_NAME"],
		stormbase:  infos.envvars["MINER_NAME"],
		keyJSON:    keyJSON,
		keyPass:    keyPass,
		gasTarget:  gasTarget,
		gasLimit:   gasLimit,
