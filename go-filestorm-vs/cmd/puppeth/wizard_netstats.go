...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for fststats availability")
	if infos, err := checkFststats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["fststats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["fststats"] = infos.Report()
		fststats = infos.config
	}
