...
// Package core implements the Filestorm consensus protocol.
package core
...
type BlockChain struct {
	...
	db     fstdb.Database // Low level persistent database to store final content in
...
// available in the database. It initialises the default Filestorm Validator and
// Processor.
func NewBlockChain(db fstdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(block *types.Block) bool) (*BlockChain, error) {
	if cacheConfig == nil {
	...
	// the head block (fstash cache or clique voting snapshot). Might as well do
	// it in advance.
...
func (bc *BlockChain) loadLastState() error {
	...
	log.Info("Loaded most recent local header", "block", currentHeader.Number, "hash", currentHeader.Hash(), "td", headerTd, "age", common.PrettyAge(time.Unix(int64(currentHeader.Time), 0)))
	log.Info("Loaded most recent local full block", "block", currentBlock.Number(), "hash", currentBlock.Hash(), "td", blockTd, "age", common.PrettyAge(time.Unix(int64(currentBlock.Time()), 0)))
	log.Info("Loaded most recent local fast block", "block", currentFastBlock.Number(), "hash", currentFastBlock.Hash(), "td", fastTd, "age", common.PrettyAge(time.Unix(int64(currentFastBlock.Time()), 0)))
...
func (bc *BlockChain) SetHead(head uint64) error {
	...
	updateFn := func(db fstdb.KeyValueWriter, header *types.Header) {
	...
	delFn := func(db fstdb.KeyValueWriter, hash common.Hash, num uint64) {
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	writeAncient := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		for i, block := range blockChain {
			...
			// Migrate all ancient blocks. This can happen if someone upgrades from Storm
	...
	writeLive := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		for i, block := range blockChain {
			...
			stats.processed++
			if batch.ValueSize() >= fstdb.IdealBatchSize {
				if err := batch.Write(); err != nil {
					return 0, err
				}
				size += batch.ValueSize()
	...
	if bc.cacheConfig.TrieDirtyDisabled {
		...
		if current := block.NumberU64(); current > TriesInMemory {
			...
			if nodes > limit || imgs > 4*1024*1024 {
				triedb.Cap(limit - fstdb.IdealBatchSize)
			}
...
func (bc *BlockChain) reorg(oldBlock, newBlock *types.Block) error {
	...
	if len(oldChain) > 0 && len(newChain) > 0 {
		logFn := log.Info
		msg := "Detected chain reorg"
		if len(oldChain) > 63 {
			msg = "Large chain reorg detected"
			logFn = log.Warn
		}
		logFn(msg, "block", commonBlock.Number(), "hash", commonBlock.Hash(),
