...
func newWorker(config *Config, chainConfig *params.ChainConfig, engine consensus.Engine, fst Backend, mux *event.TypeMux, isLocalBlock func(*types.Block) bool, init bool) *worker {
	worker := &worker{
		config:             config,
		...
		fst:                fst,
		mux:                mux,
		chain:              fst.BlockChain(),
		isLocalBlock:       isLocalBlock,
		localUncles:        make(map[common.Hash]*types.Block),
		remoteUncles:       make(map[common.Hash]*types.Block),
		unconfirmed:        newUnconfirmedBlocks(fst.BlockChain(), miningLogAtDepth),
	...
	worker.chainHeadSub = fst.BlockChain().SubscribeChainHeadEvent(worker.chainHeadCh)
	worker.chainSideSub = fst.BlockChain().SubscribeChainSideEvent(worker.chainSideCh)
...
func (w *worker) newWorkLoop(recommit time.Duration) {
	...
	for {
		...
		case <-timer.C:
			...
			if w.isRunning() && (w.chainConfig.Clique == nil || w.chainConfig.Clique.Period > 0) && w.chainConfig.Pbft == nil {
...
func (w *worker) commitNewWork(interrupt *int32, noempty bool, timestamp int64) {
	...
	pending, err := w.fst.TxPool().Pending()
	if err != nil {
	...
	for _, account := range w.fst.TxPool().Locals() {
