...
type Snapshot struct {
	config   *params.PbftConfig // Consensus engine parameters to fine tune behavior
...
// the genesis block.
func newSnapshot(config *params.PbftConfig, sigcache *lru.ARCCache, number uint64, hash common.Hash, signers []common.Address) *Snapshot {
	snap := &Snapshot{
		config:   config,
		sigcache: sigcache,
		Number:   number,
...
func loadSnapshot(config *params.PbftConfig, sigcache *lru.ARCCache, db fstdb.Database, hash common.Hash) (*Snapshot, error) {
	blob, err := db.Get(append([]byte("pbft-"), hash[:]...))
	if err != nil {
		return nil, err
	}
	snap := new(Snapshot)
...
func (s *Snapshot) store(db fstdb.Database) error {
	blob, err := json.Marshal(s)
	if err != nil {
		return err
	}
	return db.Put(append([]byte("pbft-"), s.Hash[:]...), blob)
}
