...
func ReadDatabaseVersion(db fstdb.KeyValueReader) *uint64 {
	var version uint64
...
func WriteDatabaseVersion(db fstdb.KeyValueWriter, version uint64) {
...
func ReadChainConfig(db fstdb.KeyValueReader, hash common.Hash) *params.ChainConfig {
...
func WriteChainConfig(db fstdb.KeyValueWriter, hash common.Hash, cfg *params.ChainConfig) {
	if cfg == nil {
		return
	}
...
func ReadPreimage(db fstdb.KeyValueReader, hash common.Hash) []byte {
	data, _ := db.Get(preimageKey(hash))
	return data
}

// WritePreimages writes the provided set of preimages to the database.
func WritePreimages(db fstdb.KeyValueWriter, preimages map[common.Hash][]byte) {
