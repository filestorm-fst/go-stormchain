...
var (
	...
	// AllFstashProtocolChanges contains every protocol change (EIPs) introduced
	// and accepted by the Filestorm core developers into the Fstash consensus.
	//
...
type ChainConfig struct {
	...
	// EIP150 implements the Gas price changes (https://github.com/filestorm/EIPs/issues/150)
...
type FstashConfig struct{}

// String implements the stringer interface, returning the consensus engine details.
func (c *FstashConfig) String() string {
	return "fstash"
}
...
type PbftConfig struct {
	Period uint64 `json:"period"` // Number of seconds between blocks to enforce
	Epoch  uint64 `json:"epoch"`  // Epoch length to reset votes and checkpoint
	FlushNumber uint64 `json:"FlushNumber"` //Number of blocks in one flash interval
}
...
// CheckConfigForkOrder checks that we don't "skip" any forks, storm isn't pluggable enough
