...
func newTester(t *testing.T, confOverride func(*fst.Config)) *tester {
	...
	ethConf := &fst.Config{
		...
		Miner: miner.Config{
			Stormbase: common.HexToAddress(testAddress),
		},
	...
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return fst.New(ctx, ethConf) }); err != nil {
		t.Fatalf("failed to register Filestorm protocol: %v", err)
	}
