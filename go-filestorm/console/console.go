...
import (
	...
	"github.com/filestorm/go-filestorm/internal/jsre"
	"github.com/filestorm/go-filestorm/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute Filestorm requests through
...
func (c *Console) init(preload []string) error {
	...
	c.jsre.Set("jfst", struct{}{})

	jfstObj, _ := c.jsre.Get("jfst")
	jfstObj.Object().Set("send", bridge.Send)
	jfstObj.Object().Set("sendAsync", bridge.Send)
	...
	if _, err := c.jsre.Run("var web3 = new Web3(jfst);"); err != nil {
	...
	flatten := "var fst = web3.fst; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = fst.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
	...
	if c.prompter != nil {
		...
		// original web3 callbacks. These will be called by the jfst.* methods after
		// they got the password from the user and send the original web3 request to
		// the backend.
		if obj := personal.Object(); obj != nil { // make sure the personal api is enabled over the interface
			if _, err = c.jsre.Run(`jfst.openWallet = personal.openWallet;`); err != nil {
				return fmt.Errorf("personal.openWallet: %v", err)
			}
			if _, err = c.jsre.Run(`jfst.unlockAccount = personal.unlockAccount;`); err != nil {
				return fmt.Errorf("personal.unlockAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jfst.newAccount = personal.newAccount;`); err != nil {
				return fmt.Errorf("personal.newAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jfst.sign = personal.sign;`); err != nil {
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines fst.getBalance(fst.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Storm instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	message := "Welcome to the Storm JavaScript console!\n\n"

	// Print some generic Storm metadata
	if res, err := c.jsre.Run(`
		var message = "instance: " + web3.version.node + "\n";
		try {
			message += "coinbase: " + fst.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + fst.blockNumber + " (" + new Date(1000 * fst.getBlock(fst.blockNumber).timestamp) + ")\n";
		try {
