...
import (
	...
	"github.com/filestorm/go-filestorm/accounts/scwallet"
...
// PublicFilestormAPI provides an API to access Filestorm related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicFilestormAPI struct {
	b Backend
}

// NewPublicFilestormAPI creates a new Filestorm protocol API.
func NewPublicFilestormAPI(b Backend) *PublicFilestormAPI {
	return &PublicFilestormAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicFilestormAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
	price, err := s.b.SuggestPrice(ctx)
	return (*hexutil.Big)(price), err
}

// ProtocolVersion returns the current Filestorm protocol version this node supports
func (s *PublicFilestormAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicFilestormAPI) Syncing() (interface{}, error) {
...
// Sign calculates an Filestorm ECDSA signature for:
// keccack256("\x19Filestorm Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/filestorm/go-filestorm/wiki/Management-APIs#personal_sign
...
// Note, this function is compatible with fst_sign and personal_sign. As such it recovers
// the address of:
// hash = keccak256("\x19Filestorm Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be 27 or 28 for legacy reasons.
//
// https://github.com/filestorm/go-filestorm/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid Filestorm signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the Filestorm blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Filestorm blockchain API.
...
//
// https://github.com/filestorm/wiki/wiki/JSON-RPC#fst_sign
...
// PublicDebugAPI is the collection of Filestorm APIs exposed over the public
// debugging endpoint.
...
func (api *PublicDebugAPI) SeedHash(ctx context.Context, number uint64) (string, error) {
	...
	return fmt.Sprintf("0x%x", fstash.SeedHash(number)), nil
}

// PrivateDebugAPI is the collection of Filestorm APIs exposed over the private
// debugging endpoint.
...
// Version returns the current filestorm protocol version.
