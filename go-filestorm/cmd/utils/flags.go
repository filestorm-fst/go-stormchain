...
// Package utils contains internal helper functions for go-filestorm commands.
package utils
...
var (
	...
	defaultSyncMode = fst.DefaultConfig.SyncMode
	...
	LightLegacyServFlag = cli.IntFlag{ // Deprecated in favor of light.serve, remove in 2021
		...
		Value: fst.DefaultConfig.LightServ,
	}
	...
	UltraLightServersFlag = cli.StringFlag{
		...
		Value: strings.Join(fst.DefaultConfig.UltraLightServers, ","),
	}
	UltraLightFractionFlag = cli.IntFlag{
		Name:  "ulc.fraction",
		Usage: "Minimum % of trusted ultra-light servers required to announce a new head",
		Value: fst.DefaultConfig.UltraLightFraction,
	}
	...
	// Fstash settings
	FstashCacheDirFlag = DirectoryFlag{
		Name:  "fstash.cachedir",
		Usage: "Directory to store the fstash verification caches (default = inside the datadir)",
	}
	FstashCachesInMemoryFlag = cli.IntFlag{
		Name:  "fstash.cachesinmem",
		Usage: "Number of recent fstash caches to keep in memory (16MB each)",
		Value: fst.DefaultConfig.Fstash.CachesInMem,
	}
	FstashCachesOnDiskFlag = cli.IntFlag{
		Name:  "fstash.cachesondisk",
		Usage: "Number of recent fstash caches to keep on disk (16MB each)",
		Value: fst.DefaultConfig.Fstash.CachesOnDisk,
	}
	FstashDatasetDirFlag = DirectoryFlag{
		Name:  "fstash.dagdir",
		Usage: "Directory to store the fstash mining DAGs",
		Value: DirectoryString(fst.DefaultConfig.Fstash.DatasetDir),
	}
	FstashDatasetsInMemoryFlag = cli.IntFlag{
		Name:  "fstash.dagsinmem",
		Usage: "Number of recent fstash mining DAGs to keep in memory (1+GB each)",
		Value: fst.DefaultConfig.Fstash.DatasetsInMem,
	}
	FstashDatasetsOnDiskFlag = cli.IntFlag{
		Name:  "fstash.dagsondisk",
		Usage: "Number of recent fstash mining DAGs to keep on disk (1+GB each)",
		Value: fst.DefaultConfig.Fstash.DatasetsOnDisk,
	}
	...
	TxPoolPriceLimitFlag = cli.Uint64Flag{
		...
		Value: fst.DefaultConfig.TxPool.PriceLimit,
	}
	TxPoolPriceBumpFlag = cli.Uint64Flag{
		Name:  "txpool.pricebump",
		Usage: "Price bump percentage to replace an already existing transaction",
		Value: fst.DefaultConfig.TxPool.PriceBump,
	}
	TxPoolAccountSlotsFlag = cli.Uint64Flag{
		Name:  "txpool.accountslots",
		Usage: "Minimum number of executable transaction slots guaranteed per account",
		Value: fst.DefaultConfig.TxPool.AccountSlots,
	}
	TxPoolGlobalSlotsFlag = cli.Uint64Flag{
		Name:  "txpool.globalslots",
		Usage: "Maximum number of executable transaction slots for all accounts",
		Value: fst.DefaultConfig.TxPool.GlobalSlots,
	}
	TxPoolAccountQueueFlag = cli.Uint64Flag{
		Name:  "txpool.accountqueue",
		Usage: "Maximum number of non-executable transaction slots permitted per account",
		Value: fst.DefaultConfig.TxPool.AccountQueue,
	}
	TxPoolGlobalQueueFlag = cli.Uint64Flag{
		Name:  "txpool.globalqueue",
		Usage: "Maximum number of non-executable transaction slots for all accounts",
		Value: fst.DefaultConfig.TxPool.GlobalQueue,
	}
	TxPoolLifetimeFlag = cli.DurationFlag{
		Name:  "txpool.lifetime",
		Usage: "Maximum amount of time non-executable transaction are queued",
		Value: fst.DefaultConfig.TxPool.Lifetime,
	}
	...
	MinerGasTargetFlag = cli.Uint64Flag{
		...
		Value: fst.DefaultConfig.Miner.GasFloor,
	}
	...
	MinerGasLimitFlag = cli.Uint64Flag{
		...
		Value: fst.DefaultConfig.Miner.GasCeil,
	}
	...
	MinerRecommitIntervalFlag = cli.DurationFlag{
		...
		Value: fst.DefaultConfig.Miner.Recommit,
	}
	...
	RPCGlobalGasCap = cli.Uint64Flag{
		Name:  "rpc.gascap",
		Usage: "Sets a cap on gas that can be used in fst_call/estimateGas",
	}
	// Logging and debug settings
	EthStatsURLFlag = cli.StringFlag{
		Name:  "fststats",
		Usage: "Reporting URL of a fststats service (nodename:secret@host:port)",
	}
	...
	GpoBlocksFlag = cli.IntFlag{
		...
		Value: fst.DefaultConfig.GPO.Blocks,
	}
	GpoPercentileFlag = cli.IntFlag{
		Name:  "gpopercentile",
		Usage: "Suggested gas price is the given percentile of a set of recent transaction gas prices",
		Value: fst.DefaultConfig.GPO.Percentile,
	}
...
func setLes(ctx *cli.Context, cfg *fst.Config) {
	...
	if cfg.UltraLightFraction <= 0 && cfg.UltraLightFraction > 100 {
		log.Error("Ultra light fraction is invalid", "had", cfg.UltraLightFraction, "updated", fst.DefaultConfig.UltraLightFraction)
		cfg.UltraLightFraction = fst.DefaultConfig.UltraLightFraction
	}
...
// for Storm and returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `storm account list`)")
	log.Warn("-------------------------------------------------------------------")
...
func setFstash(ctx *cli.Context, cfg *fst.Config) {
	if ctx.GlobalIsSet(FstashCacheDirFlag.Name) {
		cfg.Fstash.CacheDir = ctx.GlobalString(FstashCacheDirFlag.Name)
	}
	if ctx.GlobalIsSet(FstashDatasetDirFlag.Name) {
		cfg.Fstash.DatasetDir = ctx.GlobalString(FstashDatasetDirFlag.Name)
	}
	if ctx.GlobalIsSet(FstashCachesInMemoryFlag.Name) {
		cfg.Fstash.CachesInMem = ctx.GlobalInt(FstashCachesInMemoryFlag.Name)
	}
	if ctx.GlobalIsSet(FstashCachesOnDiskFlag.Name) {
		cfg.Fstash.CachesOnDisk = ctx.GlobalInt(FstashCachesOnDiskFlag.Name)
	}
	if ctx.GlobalIsSet(FstashDatasetsInMemoryFlag.Name) {
		cfg.Fstash.DatasetsInMem = ctx.GlobalInt(FstashDatasetsInMemoryFlag.Name)
	}
	if ctx.GlobalIsSet(FstashDatasetsOnDiskFlag.Name) {
		cfg.Fstash.DatasetsOnDisk = ctx.GlobalInt(FstashDatasetsOnDiskFlag.Name)
	}
}
...
func setWhitelist(ctx *cli.Context, cfg *fst.Config) {
...
// SetEthConfig applies fst-related command line flags to the config.
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *fst.Config) {
...
// RegisterEthService adds an Filestorm client to the stack.
func RegisterEthService(stack *node.Node, cfg *fst.Config) {
	var err error
	...
	if err != nil {
		Fatalf("Failed to register the Filestorm service: %v", err)
	}
}
...
// RegisterEthStatsService configures the Filestorm Stats daemon and adds it to
// the given node.
func RegisterEthStatsService(stack *node.Node, url string) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		// Retrieve both fst and les services
		var ethServ *fst.Filestorm
		ctx.Service(&ethServ)

		var lesServ *les.LightFilestorm
		ctx.Service(&lesServ)

		// Let fststats use whichever is not nil
		return fststats.New(url, ethServ, lesServ)
	}); err != nil {
		Fatalf("Failed to register the Filestorm Stats service: %v", err)
	}
}
...
func RegisterGraphQLService(stack *node.Node, endpoint string, cors, vhosts []string, timeouts rpc.HTTPTimeouts) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		...
		return nil, errors.New("no Filestorm service")
	}); err != nil {
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "storm.", tagsMap)
		}
	}
}
...
func MakeChainDatabase(ctx *cli.Context, stack *node.Node) fstdb.Database {
	var (
...
func MakeChain(ctx *cli.Context, stack *node.Node) (chain *core.BlockChain, chainDb fstdb.Database) {
	var err error
	...
	if config.Clique != nil {
		...
		if !ctx.GlobalBool(FakePoWFlag.Name) {
			engine = fstash.New(fstash.Config{
				CacheDir:       stack.ResolvePath(fst.DefaultConfig.Fstash.CacheDir),
				CachesInMem:    fst.DefaultConfig.Fstash.CachesInMem,
				CachesOnDisk:   fst.DefaultConfig.Fstash.CachesOnDisk,
				DatasetDir:     stack.ResolvePath(fst.DefaultConfig.Fstash.DatasetDir),
				DatasetsInMem:  fst.DefaultConfig.Fstash.DatasetsInMem,
				DatasetsOnDisk: fst.DefaultConfig.Fstash.DatasetsOnDisk,
			}, nil, false)
		}
	}
	if gcmode := ctx.GlobalString(GCModeFlag.Name); gcmode != "full" && gcmode != "archive" {
	...
	cache := &core.CacheConfig{
		TrieCleanLimit:      fst.DefaultConfig.TrieCleanCache,
		TrieCleanNoPrefetch: ctx.GlobalBool(CacheNoPrefetchFlag.Name),
		TrieDirtyLimit:      fst.DefaultConfig.TrieDirtyCache,
		TrieDirtyDisabled:   ctx.GlobalString(GCModeFlag.Name) == "archive",
		TrieTimeLimit:       fst.DefaultConfig.TrieTimeout,
	}
	if ctx.GlobalIsSet(CacheFlag.Name) || ctx.GlobalIsSet(CacheTrieFlag.Name) {
...
// new format.
//
// e.g. storm account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// storm --keystore /tmp/mykeystore --lightkdf account new
//
