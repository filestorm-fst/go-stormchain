...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Filestorm protocol")
	statsFlag   = flag.String("fststats", "", "Fststats network monitoring auth string")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Filestorm and the faucet")
)
...
type request struct {
	...
	Account common.Address     `json:"account"` // Filestorm address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Filestorm light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Filestorm protocol stack
	client *fstclient.Client   // Client connection to the Filestorm chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Filestorm light client protocol
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		cfg := fst.DefaultConfig
	...
	// Assemble the fststats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightFilestorm
			ctx.Service(&serv)
			return fststats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := fstclient.NewClient(api)
...
// close terminates the Filestorm connection and tears down the faucet.
...
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		// Retrieve the Filestorm address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			//lint:ignore ST1005 This error is to be displayed in the browser
			err = errors.New("Something funky happened, please open an issue at https://github.com/filestorm/go-filestorm/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
...
// the username, avatar URL and Filestorm address to fund on success.
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Filestorm address and profile URL.
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", common.Address{}, errors.New("No Filestorm address found to fund")
	}
	var avatar string
...
// returning the username, avatar URL and Filestorm address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain Filestorm address,
