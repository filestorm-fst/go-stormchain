...
func (api *RetestethAPI) SetChainParams(ctx context.Context, chainParams ChainParams) (bool, error) {
	...
	case "NoProof", "NoReward":
		inner = fstash.NewFaker()
	case "Fstash":
		inner = fstash.New(fstash.Config{
			CacheDir:       "fstash",
			CachesInMem:    2,
			CachesOnDisk:   3,
			DatasetsInMem:  1,
...
func (api *RetestethAPI) ClientVersion(ctx context.Context) (string, error) {
	return "Storm-" + params.VersionWithCommit(gitCommit, gitDate), nil
}
...
func retesteth(ctx *cli.Context) error {
	...
	listener, _, err := rpc.StartHTTPEndpoint(httpEndpoint, rpcAPI, []string{"test", "fst", "debug", "web3"}, cors, vhosts, rpc.DefaultHTTPTimeouts)
	if err != nil {
