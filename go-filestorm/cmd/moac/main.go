...
const (
	...
	addMicroChain    string = "0xd8709d52"
	removeMicroChain string = "0xdd2bbda4"
)
...
func addIpfsSubChainHandle(w http.ResponseWriter, r *http.Request) {
	...
	_, err := ipfsManagementOperate(removeAddMicroChainParameter(addMicroChain, subchainAddress, subchainSize), address, addressKeystore, addressPassword, toAddress)
	if err != nil {
		io.WriteString(w, reStr("fail", serverErr, nil))
		return
	}
...
func deleteIpfsSubChainHandle(w http.ResponseWriter, r *http.Request) {
	...
	_, err := ipfsManagementOperate(removeAddMicroChainParameter(removeMicroChain, subchainAddress, ""), address, addressKeystore, addressPassword, toAddress)
	if err != nil {
		io.WriteString(w, reStr("fail", serverErr, nil))
		return
	}
...
合约地址 contractAddress = 0x04e3205e01f63e7a3dbbcf8d1795499c89925d3b
*/
//addMicroChain, removeMicroChain参数拼接
func removeAddMicroChainParameter(pType, contractAddress, contractSize string) string {
