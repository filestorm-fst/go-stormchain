...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"fststats", "explorer", "wallet", "faucet"} {
		...
		for _, server := range available[service] {
			...
			switch service {
			case "fststats":
				if infos, err := checkFststats(client, w.network); err == nil {
					port = infos.port
				}
			case "explorer":
	...
	// If we have fststats running, ask whether to make the secret public or not
	if w.conf.fststats != "" {
		fmt.Println()
		fmt.Println("Include fststats secret on dashboard (y/n)? (default = yes)")
