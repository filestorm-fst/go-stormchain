...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	...
	echo $'exec storm --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --fststats \'{{.Fststats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh
...
services:
  wallet:
    ...
    ports:
      ...
      - "{{.WebPort}}:80"{{end}}
    volumes:
      - {{.Datadir}}:/root/.filestorm
    environment:
      - NODE_PORT={{.NodePort}}/tcp
      - STATS={{.Fststats}}{{if .VHost}}
      - VIRTUAL_HOST={{.VHost}}
      - VIRTUAL_PORT=80{{end}}
    logging:
      driver: "json-file"
