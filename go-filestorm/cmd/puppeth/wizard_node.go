...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.fststats == "" {
		log.Error("No fststats server configured")
		return
	}
	...
	if w.conf.Genesis.Config.Fstash != nil && !boot {
		fmt.Println()
		if infos.fstashdir == "" {
			fmt.Printf("Where should the fstash mining DAGs be stored on the remote machine?\n")
			infos.fstashdir = w.readString()
		} else {
			fmt.Printf("Where should the fstash mining DAGs be stored on the remote machine? (default = %s)\n", infos.fstashdir)
			infos.fstashdir = w.readDefaultString(infos.fstashdir)
		}
	}
	...
	if infos.fststats == "" {
		...
		infos.fststats = w.readString() + ":" + w.conf.fststats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.fststats)
		infos.fststats = w.readDefaultString(infos.fststats) + ":" + w.conf.fststats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.Fstash != nil {
			// Fstash based miners only need an etherbase to mine against
			fmt.Println()
			if infos.etherbase == "" {
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy Filestorm node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
