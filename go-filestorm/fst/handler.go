...
import (
	...
	"github.com/filestorm/go-filestorm/fst/fetcher"
...
// NewProtocolManager returns a new Filestorm sub protocol manager. The Filestorm sub protocol manages peers capable
// with the Filestorm network.
func NewProtocolManager(config *params.ChainConfig, checkpoint *params.TrustedCheckpoint, mode downloader.SyncMode, networkID uint64, mux *event.TypeMux, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb fstdb.Database, cacheLimit int, whitelist map[uint64]common.Hash) (*ProtocolManager, error) {
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Filestorm peer", "peer", id)

	// Unregister the peer from the downloader and Filestorm peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Filestorm protocol")
	...
	log.Info("Filestorm protocol stopped")
}
...
// handle is the callback invoked to manage the life cycle of an fst peer. When
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Filestorm peer connected", "name", p.Name())

	// Execute the Filestorm handshake
	var (
	...
	if err := p.Handshake(pm.networkID, td, hash, genesis.Hash(), forkid.NewID(pm.blockchain), pm.forkFilter); err != nil {
		p.Log().Debug("Filestorm handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Filestorm peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Filestorm message handling failed", "err", err)
			return err
		}
	}
}
...
// NodeInfo represents a short summary of the Filestorm sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // Filestorm network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
