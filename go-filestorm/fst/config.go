...
func init() {
	...
	if runtime.GOOS == "darwin" {
		DefaultConfig.Fstash.DatasetDir = filepath.Join(home, "Library", "Fstash")
	} else if runtime.GOOS == "windows" {
		localappdata := os.Getenv("LOCALAPPDATA")
		if localappdata != "" {
			DefaultConfig.Fstash.DatasetDir = filepath.Join(localappdata, "Fstash")
		} else {
			DefaultConfig.Fstash.DatasetDir = filepath.Join(home, "AppData", "Local", "Fstash")
		}
	} else {
		DefaultConfig.Fstash.DatasetDir = filepath.Join(home, ".fstash")
	}
}
...
type Config struct {
	...
	// If nil, the Filestorm main net block is used.
	...
	// RPCGasCap is the global gas cap for fst-call variants.
