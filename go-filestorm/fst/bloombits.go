...
const (
	// bloomServiceThreads is the number of goroutines used globally by an Filestorm
...
func (fst *Filestorm) startBloomHandlers(sectionSize uint64) {
	for i := 0; i < bloomServiceThreads; i++ {
		go func() {
			for {
				select {
				case <-fst.shutdownChan:
					return

				case request := <-fst.bloomRequests:
					task := <-request
					task.Bitsets = make([][]byte, len(task.Sections))
					for i, section := range task.Sections {
						head := rawdb.ReadCanonicalHash(fst.chainDb, (section+1)*sectionSize-1)
						if compVector, err := rawdb.ReadBloomBits(fst.chainDb, task.Bit, section, head); err == nil {
...
// for the Filestorm header bloom filters, permitting blazing fast filtering.
type BloomIndexer struct {
	size    uint64               // section size to generate bloombits for
	db      fstdb.Database       // database instance to write index data and metadata into
...
func NewBloomIndexer(db fstdb.Database, size, confirms uint64) *core.ChainIndexer {
