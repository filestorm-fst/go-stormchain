...
// PublicFilestormAPI provides an API to access Filestorm full node-related
// information.
type PublicFilestormAPI struct {
	e *Filestorm
}

// NewPublicFilestormAPI creates a new Filestorm protocol API for full nodes.
func NewPublicFilestormAPI(e *Filestorm) *PublicFilestormAPI {
	return &PublicFilestormAPI{e}
}

// Etherbase is the address that mining rewards will be send to
func (api *PublicFilestormAPI) Etherbase() (common.Address, error) {
...
func (api *PublicFilestormAPI) Hashrate() hexutil.Uint64 {
	return hexutil.Uint64(api.e.Miner().HashRate())
}

// ChainId is the EIP-155 replay-protection chain id for the current filestorm chain config.
func (api *PublicFilestormAPI) ChainId() hexutil.Uint64 {
	chainID := new(big.Int)
...
func NewPublicMinerAPI(e *Filestorm) *PublicMinerAPI {
...
func NewPrivateMinerAPI(e *Filestorm) *PrivateMinerAPI {
...
// PrivateAdminAPI is the collection of Filestorm full node-related APIs
...
// admin methods of the Filestorm service.
func NewPrivateAdminAPI(fst *Filestorm) *PrivateAdminAPI {
	return &PrivateAdminAPI{fst: fst}
}
...
func (api *PrivateAdminAPI) ExportChain(file string) (bool, error) {
	...
	if err := api.fst.BlockChain().Export(writer); err != nil {
		return false, err
	}
	return true, nil
}
...
func (api *PrivateAdminAPI) ImportChain(file string) (bool, error) {
	...
	for batch := 0; ; batch++ {
		...
		if hasAllBlocks(api.fst.BlockChain(), blocks) {
			blocks = blocks[:0]
			continue
		}
		// Import the batch and reset the buffer
		if _, err := api.fst.BlockChain().InsertChain(blocks); err != nil {
...
// PublicDebugAPI is the collection of Filestorm full node APIs exposed
...
// related public debug methods of the Filestorm service.
func NewPublicDebugAPI(fst *Filestorm) *PublicDebugAPI {
	return &PublicDebugAPI{fst: fst}
}
...
func (api *PublicDebugAPI) DumpBlock(blockNr rpc.BlockNumber) (state.Dump, error) {
	if blockNr == rpc.PendingBlockNumber {
		...
		_, stateDb := api.fst.miner.Pending()
	...
	if blockNr == rpc.LatestBlockNumber {
		...
		block = api.fst.blockchain.GetBlockByNumber(uint64(blockNr))
	}
	if block == nil {
...
// PrivateDebugAPI is the collection of Filestorm full node APIs exposed over
...
// private debug methods of the Filestorm service.
func NewPrivateDebugAPI(fst *Filestorm) *PrivateDebugAPI {
	return &PrivateDebugAPI{fst: fst}
}

// Preimage is a debug API function that returns the preimage for a sha3 hash, if known.
func (api *PrivateDebugAPI) Preimage(ctx context.Context, hash common.Hash) (hexutil.Bytes, error) {
	if preimage := rawdb.ReadPreimage(api.fst.ChainDb(), hash); preimage != nil {
		return preimage, nil
	}
...
func (api *PrivateDebugAPI) GetBadBlocks(ctx context.Context) ([]*BadBlockArgs, error) {
	...
	for i, block := range blocks {
		...
		if results[i].Block, err = fstapi.RPCMarshalBlock(block, true, true); err != nil {
...
func (api *PrivateDebugAPI) GetModifiedAccountsByNumber(startNum uint64, endNum *uint64) ([]common.Address, error) {
	...
	startBlock = api.fst.blockchain.GetBlockByNumber(startNum)
	if startBlock == nil {
	...
	if endNum == nil {
		endBlock = startBlock
		startBlock = api.fst.blockchain.GetBlockByHash(startBlock.ParentHash())
		if startBlock == nil {
			return nil, fmt.Errorf("block %x has no parent", endBlock.Number())
		}
	} else {
		endBlock = api.fst.blockchain.GetBlockByNumber(*endNum)
		if endBlock == nil {
...
func (api *PrivateDebugAPI) GetModifiedAccountsByHash(startHash common.Hash, endHash *common.Hash) ([]common.Address, error) {
	var startBlock, endBlock *types.Block
	startBlock = api.fst.blockchain.GetBlockByHash(startHash)
	if startBlock == nil {
	...
	if endHash == nil {
		...
		endBlock = api.fst.blockchain.GetBlockByHash(*endHash)
		if endBlock == nil {
...
func (api *PrivateDebugAPI) getModifiedAccounts(startBlock, endBlock *types.Block) ([]common.Address, error) {
	...
	triedb := api.fst.BlockChain().StateCache().TrieDB()
