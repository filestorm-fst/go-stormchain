...
var (
	...
	maxQueuedHeaders         = 32 * 1024                    // [fst/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	stateDB    fstdb.Database  // Database to state sync into (and deduplicate via)
	...
	// Channels
	headerCh      chan dataPack        // [fst/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [fst/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [fst/63] Channel receiving inbound receipts
	bodyWakeCh    chan bool            // [fst/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [fst/63] Channel to signal the receipt fetcher of new tasks
	headerProcCh  chan []*types.Header // [fst/62] Channel to feed the header processor new tasks

	// for stateFetcher
	stateSyncStart chan *stateSync
	trackStateReq  chan *stateReq
	stateCh        chan dataPack // [fst/63] Channel receiving inbound node state data
...
func New(checkpoint uint64, stateDb fstdb.Database, stateBloom *trie.SyncBloom, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
...
// these are zero.
func (d *Downloader) Progress() filestorm.SyncProgress {
...
func (d *Downloader) Synchronise(id string, head common.Hash, td *big.Int, mode SyncMode) error {
	...
	case errTimeout, errBadPeer, errStallingPeer, errUnsyncedPeer,
		...
		log.Info("Synchronisation slowing down", "peer", id, "msg", err)
		if d.dropPeer == nil {
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "fst", p.version, "head", hash, "td", td, "mode", d.mode)
