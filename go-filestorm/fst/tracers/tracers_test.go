...
import (
	...
	"github.com/filestorm/go-filestorm/tests"
)

// To generate a new callTracer test, copy paste the makeTest method below into
// a Storm console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = fst.getBlock(fst.getTransaction(tx).blockHash);
  var genesis = fst.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.fst.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  fst.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
