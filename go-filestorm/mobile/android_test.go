...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Storm.LightScryptN, Storm.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.filestorm", new NodeConfig());
			node.start();
			...
			FilestormClient ec = node.getFilestormClient();
	...
	// signatures too. Regression test for go-filestorm issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "storm-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/filestorm/gethtest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Storm and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.filestorm", "github.com/filestorm/go-filestorm/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "storm.aar"), "storm.aar")

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "filestorm", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.filestorm.gethtest"
