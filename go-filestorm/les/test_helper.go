...
import (
	...
	"github.com/filestorm/go-filestorm/accounts/abi/bind/backends"
	...
	"github.com/filestorm/go-filestorm/contracts/checkpointoracle/contract"
	...
	"github.com/filestorm/go-filestorm/les/flowcontrol"
...
func testIndexers(db fstdb.Database, odr light.OdrBackend, config *light.IndexerConfig) []*core.ChainIndexer {
	var indexers [3]*core.ChainIndexer
	indexers[0] = light.NewChtIndexer(db, odr, config.ChtSize, config.ChtConfirms)
	indexers[1] = fst.NewBloomIndexer(db, config.BloomSize, config.BloomConfirms)
...
func newTestClientHandler(backend *backends.SimulatedBackend, odr *LesOdr, indexers []*core.ChainIndexer, db fstdb.Database, peers *peerSet, ulcServers []string, ulcFraction int) *clientHandler {
	var (
	...
	client := &LightFilestorm{
		lesCommons: lesCommons{
			genesis:     genesis.Hash(),
			config:      &fst.Config{LightPeers: 100, NetworkId: NetworkId},
...
func newTestServerHandler(blocks int, indexers []*core.ChainIndexer, db fstdb.Database, peers *peerSet, clock mclock.Clock) (*serverHandler, *backends.SimulatedBackend) {
	var (
		gspec = core.Genesis{
			Config:   params.AllFstashProtocolChanges,
			Alloc:    core.GenesisAlloc{bankAddr: {Balance: bankFunds}},
