...
// Package les implements the Light Filestorm Subprotocol.
package les
...
func New(ctx *node.ServiceContext, config *fst.Config) (*LightFilestorm, error) {
	chainDb, err := ctx.OpenDatabase("lightchaindata", config.DatabaseCache, config.DatabaseHandles, "fst/db/chaindata/")
	if err != nil {
		return nil, err
	}
	...
	leth := &LightFilestorm{
		...
		engine:         fst.CreateConsensusEngine(ctx, chainConfig, &config.Fstash, nil, false, chainDb),
		bloomRequests:  make(chan chan *bloombits.Retrieval),
		bloomIndexer:   fst.NewBloomIndexer(chainDb, params.BloomBitsBlocksClient, params.HelperTrieConfirmations),
...
func (s *LightDummyAPI) Fsterbase() (common.Address, error) {
...
// APIs returns the collection of RPC services the filestorm package offers.
// NOTE, some of these services probably need to be moved to somewhere else.
func (s *LightFilestorm) APIs() []rpc.API {
...
func (s *LightFilestorm) ResetWithGenesisBlock(gb *types.Block) {
	s.blockchain.ResetWithGenesisBlock(gb)
}

func (s *LightFilestorm) BlockChain() *light.LightChain      { return s.blockchain }
func (s *LightFilestorm) TxPool() *light.TxPool              { return s.txPool }
func (s *LightFilestorm) Engine() consensus.Engine           { return s.engine }
func (s *LightFilestorm) LesVersion() int                    { return int(ClientProtocolVersions[0]) }
func (s *LightFilestorm) Downloader() *downloader.Downloader { return s.handler.downloader }
func (s *LightFilestorm) EventMux() *event.TypeMux           { return s.eventMux }

// Protocols implements node.Service, returning all the currently configured
// network protocols to start.
func (s *LightFilestorm) Protocols() []p2p.Protocol {
...
// light filestorm protocol implementation.
func (s *LightFilestorm) Start(srvr *p2p.Server) error {
	...
	s.netRPCService = fstapi.NewPublicNetAPI(srvr, s.config.NetworkId)
...
// Filestorm protocol.
func (s *LightFilestorm) Stop() error {
	close(s.closeCh)
	s.peers.Close()
	s.reqDist.close()
	s.odr.Stop()
	...
	s.chainDb.Close()
	s.wg.Wait()
	log.Info("Light filestorm stopped")
	return nil
}

// SetClient sets the rpc client and binds the registrar contract.
func (s *LightFilestorm) SetContractBackend(backend bind.ContractBackend) {
	if s.oracle == nil {
		return
	}
