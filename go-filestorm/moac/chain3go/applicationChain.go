...
func DeployApplicationChain(auth *bind.TransactOpts, backend bind.ContractBackend, blockSec *big.Int, flushEpoch *big.Int, initial_validators []common.Address, exchangeRate *big.Int) (common.Address, *types.Transaction, *ApplicationChain, error) {
	parsed, err := abi.JSON(strings.NewReader(ApplicationChainABI))
	if err != nil {
		return common.Address{}, nil, nil, err
	}

	address, tx, contract, err := bind.DeployContract(auth, parsed, common.FromHex(ApplicationChainBin), backend, blockSec, flushEpoch, initial_validators, exchangeRate)
	if err != nil {
		return common.Address{}, nil, nil, err
	}
