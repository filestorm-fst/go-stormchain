...
import (
	"bytes"
	"fmt"
	"github.com/filestorm/go-filestorm/moac/chain3go/lib/mcdb"
	"io/ioutil"
	"os"
	"strconv"
	"sync"
...
func newTestLDB() (*mcdb.LDBDatabase, func()) {
	dirname, err := ioutil.TempDir(os.TempDir(), "mcdb_test_")
	if err != nil {
		panic("failed to create test file: " + err.Error())
	}
	db, err := mcdb.NewLDBDatabase(dirname, 0, 0)
	if err != nil {
...
func testPutGet(db mcdb.Database, t *testing.T) {
	t.Parallel()
...
func testParallelPutGet(db mcdb.Database, t *testing.T) {
	const n = 8
