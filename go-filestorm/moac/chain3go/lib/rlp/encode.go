...
func (w *encbuf) encodeStringHeader(size int) {
	//in := big.NewInt(0)
	//inTxt := "0x" + in.SetBytes(w.str).Text(16)
	//log.Debug("[rlp/encode.go->encodeStringHeader] in " + inTxt)
	if size < 56 {
	...
	//out := big.NewInt(0)
	//outTxt := "0x" + out.SetBytes(w.str).Text(16)
	//log.Debug("[rlp/encode.go->encodeStringHeader] out " + outTxt)
}
